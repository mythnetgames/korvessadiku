...
const char *dec_conditions[5] = { "pristine", "fresh", "worn", "damaged",
		"barely visible" };

const char *dec_skills[9] = { "a lack of any real",  // 0~9
		"child-like",          // 9~19
		"rudimentary",         // 19~29
		"mediocre",            // 30~39
		"talented",            // 40~49
...
const char *dec_short_skills[9] = { "poorly",  // 0~9
		"poorly",          // 9~19
		"simply",         // 19~29
		"simply",            // 30~39
		"well",            // 40~49
...
const char *dec_size[4] = { "small", "large", "tiny", "all-encompassing" };

const char *dec_short[12] = { "(none)", "engraved", "embossed", "embroidered",
...
static char *strTimeWord[] = { "twelve", "one", "two", "three", "four", "five",
		"six", "seven", "eight", "nine", "ten", "eleven", "twelve", "one",
		"two", "three", "four", "five", "six", "seven", "eight", "nine", "ten",
		"eleven", "twelve" };

const char *fog_states[] = { "no fog", "thin fog", "thick fog", "\n" };

const char *weather_states[] = { "no rain", "chance of rain", "light rain",
		"steady rain", "heavy rain", "light snow", "steady snow", "heavy snow",
		"\n" };

const char *weather_clouds[] = { "clear sky", "light clouds", "heavy clouds",
		"overcast", "\n" };
...
const char *trap_desc_bits[] = { "can be set on exits", "can be set on objects",
		...
		"binds its victims", "causes a somatic efect", "breaks on use",
		"breaks on reset", "must be used as the trigger",
		"is able to be avoided if used as a trigger",
		"will only target one person", "will always strike even if discovered",
		"cannot be found by searching or detecting", "cannot be detected",
		"DO NOT USE", "\n" };
...
void target_sighted_reaction(CHAR_DATA * ch, CHAR_DATA * target) {
	...
	if (!is_brother(ch,
			target) && (wildlife_check (ch, target) != 3) && IS_NPC(ch)) {
		...
		if (IS_SET(ch->act, ACT_SHOOTER)) {
			...
			sprintf(buf2,
					"#3[Guardian: %s]#0 Room %d - Adding %s to overwatch list, arrows will be fired shortly.",
					ch->tname, ch->in_room, target->tname);
...
void do_point(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (exit
			&& IS_SET(exit->exit_info,
					EX_ISDOOR) && IS_SET (exit->exit_info, EX_CLOSED)
...
void do_title(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!(obj = get_obj_in_list_vis(ch, buf, ch->right_hand)) && !(obj =
	...
	skill = (ch->skills[ch->writes] * 0.50) + (ch->skills[ch->speaks] * 0.30)
...
void do_timeconvert(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (year < 2600 && year > 2460) {
		...
		temp_time += ((((year - GAME_BASE_YEAR) * GAME_SECONDS_PER_YEAR)
...
//changed for auroness version
		...
		sprintf(buf,
				...
				(game_date.hour == 0) ?
						12 :
						((game_date.hour > 12) ?
								game_date.hour - 11 : game_date.hour),
 ...
 */
char *frame_built[] = { "fragily-built", "scantly-built", "lightly-built",
		"typically-built", "heavily-built", "massively-built",
		"gigantically-built", "\n" };

char *verbal_intox[] = { "sober", "tipsy", "slightly drunk", "drunk",
		"intoxicated", "plastered" };

char *verbal_hunger[] = { "almost starved to death",
		"feeling faint from starvation", "slowly starving", "ravenous",
		"famished", "hungry", "feeling peckish", "full", "satiated",
		"absolutely stuffed" };

char *verbal_thirst[] = { "dying of thirst", "desperately thirsty",
		"quite parched", "feeling thirsty", "feeling slightly thirsty",
		"not thirsty", "sated", "nicely quenched", "completely sated" };
...
void do_compare(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!(obj1 = get_obj_in_dark(ch, arg1, ch->right_hand)) && !(obj1 =
			get_obj_in_dark(ch, arg1, ch->left_hand)) && !(obj1 =
			get_obj_in_dark(ch, arg1, ch->equip))
	...
	if (!(obj2 = get_obj_in_dark(ch, arg2, ch->right_hand)) && !(obj2 =
			get_obj_in_dark(ch, arg2, ch->left_hand)) && !(obj2 =
			get_obj_in_dark(ch, arg2, ch->equip))
	...
	sprintf(buffer, "\nCompared to #2%s#0, #2%s#0:", obj2->short_description,
	...
	*buffer = '\0';
	...
	GET_ITEM_TYPE(obj2) == ITEM_WEAPON) {
		double avg_dam_one = (((obj1->o.weapon.dice * obj1->o.weapon.sides)
				+ obj1->o.od.value[5])
				+ (obj1->o.weapon.dice + obj1->o.od.value[5])) / 2.0;
		double avg_dam_two = (((obj2->o.weapon.dice * obj2->o.weapon.sides)
				+ obj2->o.od.value[5])
		...
		for (int j = 0; j < 6; ++j) {
			if (avg_dam_one + weapon_armor_table[obj1->o.weapon.hit_type][j]
					> avg_dam_two
							+ weapon_armor_table[obj2->o.weapon.hit_type][j]) {
				sprintf(buffer + strlen(buffer),
						"\n   #6+#0 cause #2more#0 damage against %s armours",
						armor_types[j]);
			} else if (avg_dam_one
					+ weapon_armor_table[obj1->o.weapon.hit_type][j]
					< avg_dam_two
							+ weapon_armor_table[obj2->o.weapon.hit_type][j]) {
				sprintf(buffer + strlen(buffer),
						"\n   #6+#0 cause #1less#0 damage against %s armours",
						armor_types[j]);
			}
		}
	...
	GET_ITEM_TYPE(obj2) == ITEM_ARMOR) {
		 ...
		 */
		for (int j = 0; j < 5; ++j) {
			if (obj1->o.armor.armor_value
					- weapon_armor_table[j][obj1->o.armor.armor_type]
					> obj2->o.armor.armor_value
							- weapon_armor_table[j][obj2->o.armor.armor_type]) {
				sprintf(buffer + strlen(buffer),
						"\n   #6+#0 protects #2more#0 against %s damage",
						damage_types[j]);
			} else if (obj1->o.armor.armor_value
					- weapon_armor_table[j][obj1->o.armor.armor_type]
					< obj2->o.armor.armor_value
							- weapon_armor_table[j][obj2->o.armor.armor_type]) {
				sprintf(buffer + strlen(buffer),
						"\n   #6+#0 protects #1less#0 against %s damage",
						damage_types[j]);
			}
		}
...
void reading_check(CHAR_DATA * ch, OBJ_DATA * obj, WRITING_DATA * writing,
	...
	if (!ch->skills[writing->language]
			&& real_skill(ch,
					writing->script) && strcasecmp (writing->author, ch->tname) != STR_MATCH) {
		sprintf(output,
	...
	if (!decipher_script(ch, writing->script, writing->language, writing->skill)
			&& (strcasecmp(writing->author, ch->tname) != STR_MATCH
	...
	if (obj && IS_ELECTRIC(obj))
		sprintf(output, "The %s letters on #2%s#0's screen bear a message:",
				writing->ink, obj->short_description);
	else {
		if (!page)
			sprintf(output,
					"%s with %s scribed %s sigils in %s bears a message in %s:",
					obj->short_description, writing_adj(writing->skill),
					skills[writing->script], writing->ink,
			...
			sprintf(output, "On #2page %d#0, %s letters bear a message:",
					page - 1, writing->ink);
	}
...
void show_obj_to_char(OBJ_DATA * obj, CHAR_DATA * ch, int mode) {
	...
	*buffer = '\0';
	...
	if ((mode == 0) && obj->description) {
		...
		if (GET_ITEM_TYPE (obj) == ITEM_LIGHT && obj->o.light.hours
				&& obj->o.light.on)
			strcat(buffer, " #1(lit)#0");
		else if (GET_ITEM_TYPE (obj) == ITEM_CHEM_SMOKE && obj->o.smoke.hours
				&& obj->o.smoke.on)
			strcat(buffer, " #1(lit)#0");
		else if (GET_ITEM_TYPE (obj) == ITEM_E_LIGHT && obj->o.elecs.status)
			strcat(buffer, " #1(lit)#0");
	...
	if (mode == 7 && obj->description) {
		...
		if (IS_TABLE(obj)) {
			for (obj2 = obj->contains; obj2; obj2 = obj2->next_content) {
				if ((obj2->obj_flags.type_flag == ITEM_LIGHT
						...
						|| (obj2->obj_flags.type_flag == ITEM_E_LIGHT
								&& obj2->o.elecs.status)) {
		...
		if (obj->obj_flags.type_flag == ITEM_PARCHMENT) {
			...
			if (obj->writing && obj->writing->message) {
				...
				sprintf(buf, "%s#0 seems to be blank.", obj_short_desc(obj));
				sprintf(buffer, "#2%s\n", CAP(buf));
				return;
			}
		} else if (IS_BOOK(obj)) {
			...
			if ( IS_ELECTRIC( obj ) && obj->o.elecs.status == 0) {
				send_to_char(
						"The device's display is blank, it needs to be turned on.\n",
						ch);
				return;
			}
			...
			if (!obj->open || obj->open == 0) {
				...
				if (obj->book_title) {
					if (decipher_script(ch, obj->title_script,
							obj->title_language, obj->title_skill)) {
						if (IS_ELECTRIC(obj))
							sprintf(buf,
									"   #6The name of the device seems to be '%s.'",
									obj->book_title);
						else
							sprintf(buf,
									"   #6The book has been entitled '%s.'",
									obj->book_title);
					} else {
						if (IS_ELECTRIC(obj))
							sprintf(buf,
									"   #6You cannot make out the device's name.");
						else
							sprintf(buf,
			...
			if (writing && writing->message) {
				if (strcasecmp(writing->message, "blank") != STR_MATCH) {
					...
					if (IS_ELECTRIC(obj))
						sprintf(buf,
								"This file block has yet to be initialized.\n");
					else
			...
			if (mode == 5 && obj->full_description && *obj->full_description) {
				...
				if (obj->dec_desc && obj->dec_style) {
					for (size_t y = 0; y < strlen(obj->full_description) - 1;
							y++) {
						sprintf(buf2 + strlen(buf2), "%c",
								obj->full_description[y]);
					}

					sprintf(buf,
							"%s With %s skill and %s, this item has a %s and %s decoration of: %s",
							buf2,
							(obj->dec_quality >= 75 ? dec_skills[8] :
								obj->dec_quality >= 70 ? dec_skills[7] :
								obj->dec_quality >= 60 ? dec_skills[6] :
								obj->dec_quality >= 50 ? dec_skills[5] :
								obj->dec_quality >= 40 ? dec_skills[4] :
								obj->dec_quality >= 30 ? dec_skills[3] :
								obj->dec_quality >= 20 ? dec_skills[2] :
								obj->dec_quality >= 10 ?
										dec_skills[1] : dec_skills[0]),
							obj->dec_style, dec_size[obj->dec_size],
							dec_conditions[obj->dec_condition], obj->dec_desc);
				...
				*buf2 = '\0';
				...
				for (i = 0; i < MAX_HITLOC; i++) {
					if (IS_SET(obj->o.od.value[2], (1 << i))) {
						if (!*buf2)
							sprintf(buf2 + strlen(buf2), "#0%s#6",
									armor_locs[i]);
						...
						// then it's obviously the last bit. Hah-hah!
							...
							sprintf(buf2 + strlen(buf2), " and #0%s#6",
									armor_locs[i]);
						else
							sprintf(buf2 + strlen(buf2), ", #0%s#6",
									armor_locs[i]);
			...
			if (obj->damage || has_enviro_conds(obj) || obj->scent) {
				if (mode == 15) {
					sprintf(buffer + strlen(buffer), "%s",
							object__examine_damage(obj, 0));
				} else {
					sprintf(buffer + strlen(buffer), "%s",
							object__examine_damage(obj, 1));
				}
			}
			...
			if (GET_ITEM_TYPE (obj) == ITEM_SMALLGOOD && mode == 15) {
				sprintf(buffer + strlen(buffer),
						...
						obj->o.od.value[0],
						smallgood_types[obj->o.od.value[1]]);
			}

			if (GET_ITEM_TYPE (obj) == ITEM_COMPONENT && mode == 15
					&& obj->o.od.value[2]) {
				sprintf(buffer + strlen(buffer),
			...
			if (GET_ITEM_TYPE (obj) == ITEM_TOOL && mode == 15) {
				...
				if (obj->o.od.value[0] >= 0 || obj->o.od.value[3]
						|| obj->o.od.value[4]) {

					if (obj->o.od.value[0] >= 0) {
						sprintf(buf + strlen(buf), " has #0%d#6 uses remaining",
								obj->o.od.value[0]);
						first = false;
					}
					...
					if (obj->o.od.value[3]) {
						sprintf(buf + strlen(buf),
								"%s requires at least a #0%s Artitry#6 skill to use",
								(first ? "" : ","),
								skill_lev(obj->o.od.value[3]));
						first = false;
					}

					if (obj->o.od.value[4]) {
						sprintf(buf + strlen(buf),
								"%s can be used more ingeniously until a #0%s Artistry#6 level",
								(first ? "" : ","),
								skill_lev(obj->o.od.value[4]));
						first = false;
					}
				}
				...
				if (first)
					sprintf(buf + strlen(buf),
							" can decorate items of the following material type/s: ");
				else
					sprintf(buf + strlen(buf),
			...
			if (GET_ITEM_TYPE (obj) == ITEM_CLUSTER && mode == 15) {
				i =
						(obj->o.od.value[0] =
								0 ? SKILL_FORAGE : obj->o.od.value[0]);
				if (obj->o.od.value[0]) {
					sprintf(buf + strlen(buf),
							"\n   #6The #0%s#6 skill will assist in gathering from this object.",
							skills[obj->o.od.value[0]]);
				}

				if (vtoo(
						obj->o.od.value[2]) && !IS_SET(obj->o.od.value[1], CLUS_ONE_HIDE)) {
					if (i == SKILL_FORAGE
							|| (IS_SET(obj->o.od.value[1], CLUS_ONE_SKILLED)
									&& ch->skills[i] >= 10)) {
						sprintf(buf + strlen(buf),
								" You could attempt to gather #2%s#6 from this object.",
								obj_short_desc(vtoo(obj->o.od.value[2])));
					}
				}

				if (vtoo(
						obj->o.od.value[4]) && !IS_SET(obj->o.od.value[1], CLUS_TWO_HIDE)) {
					if (i == SKILL_FORAGE
							|| (IS_SET(obj->o.od.value[1], CLUS_TWO_SKILLED)
									&& ch->skills[i] >= 10)) {
						sprintf(buf + strlen(buf),
								" You could attempt to gather #2%s#6 from this object.",
								obj_short_desc(vtoo(obj->o.od.value[4])));
					}
				}
			...
			if (GET_ITEM_TYPE (obj) == ITEM_PROGRESS && mode == 15
				...
				sprintf(buffer + strlen(buffer),
						...
						(obj->o.od.value[0] >= obj->o.od.value[1] ?
								100 :
								((obj->o.od.value[0] * 100) / obj->o.od.value[1])));
			...
			if (GET_ITEM_TYPE (obj) == ITEM_FIREARM && mode == 15) {
				...
				if (!IS_SLING(obj)) {
					...
					sprintf(buf,
							"#6You'd guess it to be a #0%s#6 type ranged weapon that %s. ",
							skills[obj->o.firearm.use_skill],
							(obj->o.firearm.handedness == 3 ?
									"is designed to be fired from #0both hands#6" :
								obj->o.firearm.handedness == 2 ?
										"is designed to be fired from #0one hand#6" :
										"is designed to be fired from either #0one or both hands#6"));

					if (!IS_DIRECT(obj)
							&& IS_SET(obj->o.firearm.bits, GUN_DISPLAY)
							&& ((obj->equiped_by && obj->equiped_by == ch)
									|| !obj->equiped_by)) {
					...
					*buf4 = '\0';
					...
					sprintf(buf + strlen(buf),
							"This weapon takes #0%s%s#6 missiles.#0", buf4,
							calibers[obj->o.firearm.caliber]);
					...
					if ((obj->equiped_by && obj->equiped_by == ch)
							|| !obj->equiped_by) {
						if (obj->contains) {
							if (IS_DIRECT(obj)) {
			...
			if (GET_ITEM_TYPE (obj) == ITEM_CLIP && mode == 15) {
				sprintf(buf, "#6This %s can hold #0%s %s %s #6 round%s",
						gun_bits[obj->o.clip.type],
						verbal_number[obj->o.clip.max],
						calibers[obj->o.clip.caliber],
						ammo_sizes[obj->o.clip.size],
						(obj->o.clip.max > 1 ? "s" : ""));
				...
				if (obj->contains) {
					...
					*buf2 = '\0';
					...
					for (tobj = obj->contains; tobj; tobj =
							tobj->next_content) {
			...
			if (GET_ITEM_TYPE (obj) == ITEM_ROUND && mode == 15) {
				...
				sprintf(buf, "#6%s #0%s %s#6 missile%s.#0",
						(obj->count > 1 ? "These are" : "This is a"),
						ammo_bits[obj->o.bullet.type],
						calibers[obj->o.bullet.caliber],
						(obj->count > 1 ? "s" : ""));
			...
			if (GET_ITEM_TYPE (obj) == ITEM_BULLET && mode == 15) {
				...
				sprintf(buf, "#6%s #0%s %s#6 bullet%s.#0",
						(obj->count > 1 ? "These are spent" : "This is a spent"),
						calibers[obj->o.bullet.caliber],
						ammo_sizes[obj->o.bullet.size],
						(obj->count > 1 ? "s" : ""));
			...
			if (GET_ITEM_TYPE (obj) == ITEM_COVER && mode == 15) {
				*buf2 = '\0';
				...
				sprintf(buf,
						"#6It would provide seating for up to #0%s %s#6 and #0%s cover#6 from #0%s#6 of the following directions:#0%s",
						verbal_number[obj->o.od.value[5]],
						(obj->o.od.value[5] == 1 ? "person" : "people"),
						(obj->o.od.value[1] >= 2 ? "good" :
							obj->o.od.value[1] == 1 ? "ordinary" : "poor"),
						(obj->o.od.value[2] == 0 ?
								"all" : verbal_number[obj->o.od.value[2]]),
						buf2);
			...
			if (GET_ITEM_TYPE (obj) == ITEM_WEAPON && mode == 15) {
				...
				sprintf(buffer + strlen(buffer),
						...
						obj->o.od.value[0]);
			} else if (GET_ITEM_TYPE (obj) == ITEM_ARTWORK && obj->o.od.value[1]
					&& obj->nVirtual == 5065 && mode == 15) {

			} else if (GET_ITEM_TYPE (obj) == ITEM_CARD
					&& (mode == 15 || mode == 0)) {

				if ((obj->o.od.value[4] == 1
						&& (ch == obj->equiped_by || ch == obj->carried_by))
						|| obj->o.od.value[4] == 2) {
					*buf = '\0';
					sprintf(buffer + strlen(buffer),
							"\n   #6This pile has the following #0%s#6 cards:#0",
							verbal_number[obj->o.od.value[5]]);
					...
					sprintf(buffer + strlen(buffer),
							"\n   #6This pile has #0%s#6 cards, but you are unable to see their values.#0",
							verbal_number[obj->o.od.value[5]]);
			}
			 ...
			 */
				...
				sprintf(buffer + strlen(buffer),
						"\n   #6This is a #0%s#6 type power pack.#0",
						batt_bits[obj->o.battery.type]);
				if (!obj->o.battery.power) {
					sprintf(buffer + strlen(buffer),
							"#6 The indicator patch is blank,\nindicating the battery is out of power.#0");
				}
			} else if (IS_ELECTRIC(obj) && mode == 15) {
				// First off - a generic "is the power on or not"?
				sprintf(buf + strlen(buf), "\n   #6Is it switched #0%s#6",
						(obj->o.elecs.status != 0 ? "on" : "off"));

				// Then, if the object is so enabled, show how much power is left.
				if ((obj->o.elecs.status != 0) && obj->contains
						&& IS_SET(obj->o.elecs.elec_bits, ELEC_FEA_POWER)) {
					sprintf(buf + strlen(buf),
							", and has about #0%d#6 hours of power left",
							obj->contains->o.battery.power);
				}
				...
				*buf2 = '\0';
				...
				if (IS_SET(obj->o.elecs.elec_bits, ELEC_FEA_WATERPROOF)) {
					sprintf(buf + strlen(buf),
							". It looks reasonably waterproof");
				}
				...
				case ITEM_E_CLOCK:
					if (obj->o.elecs.status != 0) {
						int minutes = 0;
						minutes = 4 * (15 * 60 - (next_hour_update - time(0)))
								/ 60;
						sprintf(buf + strlen(buf),
								". The clock display reads: #6%d:%s%d of %s#0",
								time_info.hour, minutes >= 10 ? "" : "0",
								minutes, earth_phase[time_info.phaseEarth]);
					}
					break;
				case ITEM_E_REMOTE:
					if (obj->o.elecs.status != 0) {
						if (obj->o.od.value[3]) {
							if (obj->o.od.value[4]
									&& get_char_id(obj->o.od.value[4])) {
								sprintf(buf + strlen(buf),
										". It is monitoring #5%s#0",
										get_char_id(obj->o.od.value[3])->short_descr);
							} else if (get_char_id(obj->o.od.value[3])) {
								sprintf(buf + strlen(buf),
										". It is synchronised to #5%s#0",
										get_char_id(obj->o.od.value[3])->short_descr);

								get_char_id(obj->o.od.value[3])->mob->controller =
										obj->coldload_id;
							} else {
								sprintf(buf + strlen(buf),
										". It appears that it thinks it is synchronised to something, but the data is corrupt - switching it off and then on would fix it");
							}
						}
					}
					break;
				...
				case ITEM_E_BUG:
					if (obj->o.elecs.status != 0) {
						...
						if (obj->in_room > 0 || ch == obj->equiped_by
								|| ch == obj->carried_by) {
							sprintf(buf + strlen(buf),
									". It is set to transmit over channel #0%d#6",
									obj->o.radio.channel);

							if (IS_SET(obj->o.elecs.elec_bits,
									ELEC_FEA_RADIO_ENCRYPT)
									&& obj->o.radio.encrypt)
								sprintf(buf + strlen(buf),
										" and is using an encryption code: #0%d#6",
										obj->o.radio.encrypt);
						}

						if (IS_SET(obj->o.elecs.elec_bits, ELEC_FEA_RADIO_SCAN))
							sprintf(buf + strlen(buf),
									". It can also scan for active frequencies");

						if (IS_SET(obj->o.elecs.elec_bits,
								ELEC_FEA_RADIO_ONEWAY))
							sprintf(buf + strlen(buf),
									". It can receive, but cannot send transmissions");

					}
					break;
				case ITEM_E_BOOST:
					if (obj->o.elecs.status != 0) {
						if (obj->o.od.value[3] && obj->o.od.value[5]
						...
						if (obj->o.od.value[4] && obj->o.od.value[5]
				...
				sprintf(buffer + strlen(buffer),
						...
						(obj->o.od.value[1] > 5 ? damage_severity[5] :
							obj->o.od.value[1] < 0 ?
									damage_severity[0] :
									damage_severity[obj->o.od.value[1]]));
				...
				sprintf(buffer + strlen(buffer),
						"\n   #6Uses Remaining:#0 %s\n",
						(x >= 600 ? "Hundreds" : x >= 400 ? "Dozens" :
							x >= 300 ? "Many" : x >= 200 ? "Quite a lot" :
							x >= 100 ? "Several" : x >= 50 ? "A few" :
							x > 0 ? "Barely any" : "None"));
			}

			if ((trap = obj->trap) && trap->trap_bit
					&& !IS_SET(trap->trap_bit, TRAP_MISC_MYSTERY)
					&& mode == 15) {
				*buf2 = '\0';
				...
				sprintf(buf,
						"This item can be used as a component for a trap, and has the following characteristics: %s",
						buf2);
				...
				if (trap->com_target_dice == -2)
					...
					sprintf(buf2, "#0%dd%d#6 people of the victim's #0group#6",
							trap->com_target_dice, trap->com_target_side);
				else
				...
				sprintf(buf + strlen(buf),
						". It would be #0%s#6 to use as part of a trap",
						trap->com_diff >= 3 ? "very difficult" :
						trap->com_diff == 2 ? "difficult" :
						trap->com_diff == 1 ? "somewhat difficult" : "easy");

				if (trap->com_trig_shots > 0)
					sprintf(buf + strlen(buf),
							", and if used as a trigger, it can be used #0%d#6 time%s before needing to be reset",
							trap->com_trig_shots,
							(trap->com_trig_shots > 1 ? "s" : ""));

				if (trap->dam1_dice && trap->dam1_sides) {
					dam = ((trap->dam1_dice)
							+ (trap->dam1_dice * trap->dam1_sides)) / 2;
					sprintf(buf + strlen(buf),
							". It will inflict %s #0%s#6 #0%s%s#6 to the #0%s#6",
							(trap->dam1_times > 2 ? "#0multiple#6" :
								trap->dam1_times > 1 ? "a #0pair#6 of" : "a"),
							(dam > 12 ? "huge" : dam > 8 ? "large" :
								dam > 5 ? "moderate" :
								dam > 3 ? "minor" : "small"),
							fight_damage[trap->dam1_type],
							(trap->dam1_times > 1 ? "s" : ""),
							(trap->dam1_loc > 7 ?
									"victim" : body_tab[0][trap->dam1_loc].part));

					if (trap->dam2_dice && trap->dam2_sides) {
						dam = ((trap->dam2_dice)
								+ (trap->dam2_dice * trap->dam2_sides)) / 2;
						sprintf(buf + strlen(buf),
								", and %s #0%s#6 #0%s%s#6 to the #0%s#6",
								(trap->dam2_times > 2 ? "#0multiple#6" :
									trap->dam2_times > 1 ? "a #0pair#6 of" : "a"),
								(dam > 12 ? "huge" : dam > 8 ? "large" :
									dam > 5 ? "moderate" :
									dam > 3 ? "minor" : "small"),
								fight_damage[trap->dam2_type],
								(trap->dam2_times > 1 ? "s" : ""),
								(trap->dam2_loc > 7 ?
										"victim" :
										body_tab[0][trap->dam2_loc].part));
					}
				}
					...
					if (af->type == MAGIC_HIDDEN) {
						if (af->a.hidden.clan > 0) {
							...
							if (clan && is_clan_member_player(ch, clan)) {
								sprintf(buf,
										"   Near this hidden object is the mark of %s.\n",
										clan);
								reformat_string(buf, &p);
								strcat(buffer, "\n");
								strcat(buffer, p);
			...
			*buf = '\0';
			...
			if (!IS_MORTAL(ch)) {
				for (tcraft = crafts; tcraft; tcraft = tcraft->next) {
					...
					if (crafts_found)
						sprintf(buf + strlen(buf), ", ");
					sprintf(buf + strlen(buf), "'%s %s'", tcraft->command,
			...
			if (IS_TABLE(obj)) {
				for (obj2 = obj->contains; obj2; obj2 = obj2->next_content) {
					if (CAN_SEE_OBJ(ch, obj2)) {
						...
						else if (GET_ITEM_TYPE (obj2) == ITEM_CHEM_SMOKE
								&& obj2->o.smoke.hours && obj2->o.smoke.on)
							strcat(buffer + strlen(buffer), " #1(lit)#0\n");
	...
	if (mode == 1 && GET_ITEM_TYPE (obj) == ITEM_E_LIGHT
			&& obj->o.elecs.status) {
		if (obj->o.light.hours && obj->o.light.on)
			sprintf(buffer + strlen(buffer), " #1(lit)#0");
	}
	...
	if (mode == 1 && GET_ITEM_TYPE (obj) == ITEM_TOOL && obj->o.od.value[0] >= 0
...
int list_obj_to_char(OBJ_DATA * list, CHAR_DATA * ch, int mode, int show) {
	OBJ_DATA *i = NULL;
	int found = 0, j = 0;
	...
	for (i = list; i; i = i->next_content) {
		if ((show == 7 || show == 10)
				&& (OBJ_MASS(i) <= 1 || !could_see_obj(ch, i)))
			continue;
		else if ((show == 8 || show == 11)
				&& (OBJ_MASS(i) <= 1000 || !could_see_obj(ch, i)))
			continue;
		else if ((show == 9 || show == 12)
				&& (OBJ_MASS(i) <= 2500 || !could_see_obj(ch, i)))
			continue;

		j++;
	}

	if (j >= 25 && show != 4 && show != 10 && show != 11 && show != 12
			&& !list->in_obj)
		clump = true;
		...
		if (could_see_obj(ch, i)) {
			...
			if (!mode && !looked_for_tables && IS_FURNISH(i)) {
				...
				if (looked_for_tables == 1)
					...
					send_to_char(
							"#6There are a couple of furnishings here.#0\n",
							ch);
				else
			...
			if (!mode && !looked_for_casings && GET_ITEM_TYPE(i) == ITEM_CASE) {
				...
				if (looked_for_casings == 1)
					...
					send_to_char(
							"#6Spent casings litter the ground underfoot.#0\n",
							ch);
				else if (looked_for_casings < 200)
					send_to_char(
							"#6Dozens of spent casings blanket the area.#0\n",
							ch);
				else
					send_to_char(
							"#6The area is buried by hundreds of spent casings.#0\n",
							ch);
			}
			...
			if (!mode && !looked_for_bullets && GET_ITEM_TYPE(i) == ITEM_BULLET) {
				...
				if (looked_for_bullets == 1)
					...
					send_to_char(
							"#6A couple of discharged bullets are here.#0\n",
							ch);
				else if (looked_for_bullets < 24)
					send_to_char(
							"#6The area is studded with several discharged bullets.#0\n",
							ch);
				else if (looked_for_bullets < 200)
					send_to_char(
							"#6Dozens of discharged bullets have peppered the area.#0\n",
							ch);
				else
					send_to_char(
							"#6Hundreds of discharged bullets have perforated nearly everything in the area.#0\n",
							ch);
			}
			...
			if (!mode && !looked_for_corpses && i->nVirtual == VNUM_CORPSE) {
				...
				if (looked_for_corpses == 1)
					...
					send_to_char(
							"#6There are a number of corpses littering the area.#0\n",
							ch);
				else
					send_to_char(
							"#6There are mounds of corpses strewn everywhere.#0\n",
							ch);

			}

			if (!mode && !looked_for_limbs
					&& (i->nVirtual == VNUM_WHOLEARM
							|| i->nVirtual == VNUM_WHOLELEG
							|| i->nVirtual == VNUM_LOWERTORSO
							|| i->nVirtual == VNUM_UPPERTORSO
							|| i->nVirtual == VNUM_ZOMBIE_HEAD)) {
				...
				if (looked_for_limbs == 1)
					...
					send_to_char(
							"#6Blood, gore and limbs are strewn across the area.#0\n",
							ch);
				else
					send_to_char(
							"#6Masses of savaged, rotting flesh and limbs infests the area.#0\n",
							ch);
			}
	...
	if (clump) {
		if (j >= 25 && j < 35)
			...
			send_to_char(
					...
					ch);
	} else if (!found && show && show != 4 && show != 7 && show != 8
			&& show != 9 && show != 10 && show != 11 && show != 12)
		send_to_char("Nothing.\n", ch);
...
int list_char_to_char(CHAR_DATA * list, CHAR_DATA * ch, int mode) {
	...
	if (j
			< 25|| mode == 4 || mode == 10 || mode == 11 || mode == 12 || ch->room->vnum == AMPITHEATRE) {
		for (i = list; i; i = i->next_in_room) {
...
int enter_exit_msg(CHAR_DATA * ch, char *buffer) {
	...
	char *e_dirs[] = { "to the north", "to the east", "to the south",
			"to the west", "above", "below", "outside", "inside",
			"to the northeast", "to the northwest", "to the southeast",
			"to the southwest", "to the upper north", "to the upper east",
			"to the upper south", "to the upper west", "to the upper northeast",
			"to the upper northwest", "to the upper southeast",
			"to the upper southwest", "to the lower north", "to the lower east",
			"to the lower south", "to the lower west", "to the lower northeast",
			"to the lower northwest", "to the lower southeast",
			"to the lower southwest" };
	char *a_dirs[] = { "the south", "the west", "the north", "the east",
			"below", "above", "inside", "outside", "the southwest",
			"the southeast", "the northwest", "the northeast", "the upnorth",
			"the upeast", "the upsouth", "the upwest", "the upnortheast",
			"the upnorthwest", "the upsoutheast", "the upsouthwest",
			"the downnorth", "the downeast", "the downsouth", "the downwest",
			"the downnortheast", "the downnorthwest", "the downsoutheast",
			"the downsouthwest", "report this to nimrod" };
	...
	if (get_second_affect(ch, SA_LEAD, NULL) && !ch->following) {
		sprintf(addon, "#3%s is %s %s%s%s, a %s following behind.#0",
				CAP(char_short(ch)), (isLeaving ? "leaving" : "arriving from"),
				(isLeaving ? e_dirs[qe->dir] : a_dirs[qe->dir]),
				((qe->travel_str == NULL && ch->travel_str) ? ", " : ""),
				((qe->travel_str ?
						qe->travel_str : (ch->travel_str ? ch->travel_str : ""))),
				qe->group_str);
	} else {
		sprintf(addon, "#3%s is %s %s%s%s.#0", CAP(char_short(ch)),
...
char_short(CHAR_DATA * ch) {
	...
	if (!IS_NPC(ch) && !IS_SET(ch->flags, FLAG_GUEST) && in_arena_room(ch)
			&& is_clan_member(ch, "arena_blue"))
		arena_color = 1;
	else if (!IS_NPC(ch) && !IS_SET(ch->flags, FLAG_GUEST) && in_arena_room(ch)
			&& is_clan_member(ch, "arena_red"))
		arena_color = 2;
	...
	if (arena_color) {
		...
		sprintf(buf, "%s %s%s %s", arg1,
				arena_color == 2 ? "red-painted" : "blue-painted",
				(arg3[strlen(arg3) - 1] == ',' ? "," : ""), arg2);

		return buf;
	}

	int z = armor_descript(ch);
	sprintf(phrase, "%s%s%s",
			(z >= 4 ? "extremely well armoured" : z == 3 ? "heavily armoured" :
				z == 2 ? "well armoured" : z == 1 ? "lightly armoured" : ""),
			(z ? ", " : ""), height_phrase(ch));
...
char_long(CHAR_DATA * ch, int show_tname) {
	...
	if ((obj = get_equip(ch, WEAR_NECK_1))
			&& IS_SET(obj->obj_flags.extra_flags,
					ITEM_MASK) && IS_SET (ch->affected_by, AFF_HOODED)) {
		if (ch->pmote_str) {
			if (*(ch->pmote_str) == '\'') {
				sprintf(buf, "A %s, %s person%s %s", phrase, obj->desc_keys,
						tname, ch->pmote_str);
				return buf;
			} else {
				sprintf(buf, "A %s, %s person%s %s", phrase, obj->desc_keys,
			...
			sprintf(buf, "A %s, %s person %sis here.", phrase, obj->desc_keys,
					tname);
			return buf;
		}
	}
	...
	if (((obj = get_equip(ch, WEAR_HEAD))
		...
		if (obj->obj_flags.type_flag == ITEM_WORN)
			if (ch->pmote_str) {
				if (*(ch->pmote_str) == '\'')
					sprintf(buf, "A %s, %s %sperson%s", phrase, obj->desc_keys,
							tname, ch->pmote_str);
				else
					sprintf(buf, "A %s, %s %sperson %s", phrase, obj->desc_keys,
							tname, ch->pmote_str);
...
void show_char_to_char(CHAR_DATA * i, CHAR_DATA * ch, int mode) {
	...
	if (mode == 4 || mode == 7 || mode == 10 || mode == 8 || mode == 9
			|| mode == 11 || mode == 12)
		mode = 0;
	...
	if (!mode) {
		...
		if ((blindfold = get_equip(i, WEAR_BLINDFOLD))
						...
						*buf3 = '\0';
						...
						for (af = i->hour_affects; af; af = af->next) {
							if (af->type == AFFECT_COVER) {
								...
								if (j == 1)
									sprintf(buf3, " %s",
											(af->a.cover.direction == 6 ?
													"area" :
													dirs[af->a.cover.direction]));
								else if (j == x)
									sprintf(buf3 + strlen(buf3), " and %s",
											(af->a.cover.direction == 6 ?
													"area" :
													dirs[af->a.cover.direction]));
								else
						...
						if ((af = get_affect(i, AFFECT_COVER))
								&& is_obj_in_list(af->a.table.obj,
										i->room->contents))
							sprintf(buf4,
									"taking cover from the%s behind #2%s#0.",
									buf3, obj_short_desc(af->a.table.obj));
						else
			...
			if (IS_SUBDUER(i)) {
				if (IS_RIDER(i))
					...
					sprintf(buffer, "#5%s#0 has #5%s#0 in tow.", char_short(i),
							i->subdue == ch ? "you" : char_short(i->subdue));
				buffer[2] = toupper(buffer[2]);

				if (do_group_size(i) > 9 && !i->following) {
					sprintf(buffer + strlen(buffer),
							" #5A %s#0 stands behind %s.", group,
							sex_noun[(int) i->sex]);
				}
				...
				sprintf(buffer, "%s#0 %s %s#5%s#0.", char_short(i),
				...
				*buffer = '\0';
				...
				if ((GET_FLAG (i, FLAG_ENTERING) || GET_FLAG(i, FLAG_LEAVING))
						&& enter_exit_msg(i, buffer)) {
					if (!IS_NPC(i) && IS_SET(i->plr_flags, NEW_PLAYER_TAG))
						strcat(buffer, " #2(new player)#0");

					if (!IS_NPC(
							i) && IS_GUIDE(i) && IS_SET (i->flags, FLAG_GUEST))
						strcat(buffer, " #B(new player guide)#0");
					...
					if (!((do_group_size(i) > 5
							|| get_second_affect(i, SA_GFOLLOW, NULL))
							&& i->following))
						send_to_char(p, ch);
		...
		if (i->scent) {
			...
			for (scent = i->scent; scent; scent = scent->next) {
				...
				if ((ch == i && scent_strength(scent))
						|| scent_strength(scent) >= 2) {
					if (!scent_one) {
						scent_one = scent;
						scent_count++;
					} else if (!scent_two) {
						...
						scent_three = scent;
						scent_count++;
					} else if (scent_strength(scent)
							> scent_strength(scent_one)) {
						scent_one = scent;
					} else if (scent_strength(scent)
							> scent_strength(scent_two)) {
						scent_two = scent;
					} else if (scent_strength(scent)
							> scent_strength(scent_two)) {
						scent_three = scent;
			...
			if (scent_count == 3) {
				sprintf(stink_buf,
						"You detect %s of %s, %s of %s and %s of %s about %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref),
						scent_tier[scent_strength(scent_three)],
						scent_lookup(scent_three->scent_ref), HMHR(i));
			} else if (scent_count == 2) {
				sprintf(stink_buf,
						"You detect %s of %s and %s of %s about %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref), HMHR(i));
			} else if (scent_count == 1) {
				sprintf(stink_buf, "You detect %s of %s about %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref), HMHR(i));
			}
		}
		...
		*buf3 = '\0';
		...
		if (mode
				== 15&& *buf2 && (is_leader (ch, i) || i->following == ch) && IS_SET (i->act, ACT_AGGRESSIVE) && IS_NPC(i)) {
		...
		if (mode
				== 15&& *buf2 && (is_leader (ch, i) || i->following == ch) && !IS_SET (i->act, ACT_SENTINEL) && IS_NPC(i)) {
		...
		if (mode == 15
				&& !str_cmp(lookup_race_variable(i->race, RACE_NAME), "robot")
				&& i->morph_time) {
			...
			if (hours > 1) {
				sprintf(buf,
						"The display on this robot indicates it has about %d hours of power left.\n",
						hours);
			} else {
				sprintf(buf,
						"The display on this robot indicates it has less than an hour of power left.\n");
			}
		...
		if (mode == 15 && i->damage && !IS_SET(i->act, ACT_VEHICLE)
			...
			if (curdamage > 0 && curdamage <= i->max_hit * .25)
				sprintf(buffer, "%s face looks slightly pale", char_short(i));
			else if (curdamage > i->max_hit * .25
					&& curdamage < i->max_hit * .50)
				sprintf(buffer, "%s face looks rather pallid", char_short(i));
			...
			if (get_affect(i, AFFECT_INTERNAL)
					&& ch->skills[SKILL_FIRSTAID] >= 10) {
				sprintf(buffer + strlen(buffer),
						" and %s is suffering from internal injuries", HSSH(i));
			}
		...
		if (i->mob && i->mob->vehicle_type == VEHICLE_HITCH && (troom =
				vnum_to_room(i->mob->vnum))
		...
		for (enchantment = i->enchantments; enchantment; enchantment =
				enchantment->next) {
		...
		*buf3 = '\0';
		if (i == ch) {
			...
			for (location = 0; location < MAX_WEAR; location++) {
				...
				if ((location == WEAR_BLINDFOLD || GET_TRUST (ch) >= 4)
						|| (IS_OBJ_VIS(ch, eq)
								&& !IS_SET(eq->obj_flags.extra_flags2,
										ITEM_CONCEALED))) {
...
void do_search(CHAR_DATA * ch, char *argument, int cmd) {
	...
	act("$n carefully searches the area.", true, ch, 0, 0,
			TO_ROOM | _ACT_SEARCH);
...
void delayed_search(CHAR_DATA * ch) {
	...
	if (((helm = get_equip(ch, WEAR_HEAD))
			&& IS_SET(helm->obj_flags.extra_flags, ITEM_MASK))
			|| ((helm = get_equip(ch, WEAR_FACE))
					&& IS_SET(helm->obj_flags.extra_flags, ITEM_MASK)))
	...
	if (dir == -1) {
		...
		for (tch = ch->room->people; tch; tch = tch->next_in_room) {
			...
			if (!IS_LIGHT(
					ch->room) && !get_affect (ch, MAGIC_AFFECT_INFRAVISION)
					&& !is_goggled(ch)
		...
		if (!somebody_found) {
			...
			act("$n finishes searching.", true, ch, 0, 0,
					TO_ROOM | _ACT_SEARCH);
		}
		...
		for (obj = ch->room->contents; obj; obj = obj->next_content) {
			...
			for (taf = obj->xaffected; taf; taf = taf->next) {
				if ((taf->type == MAGIC_HIDDEN)
						&& (taf->a.hidden.coldload_id == ch->coldload_id))
					known = true;
			}
			...
			if (af->a.hidden.hidden_value
					&& number(1, search_base)
							< number(1, af->a.hidden.hidden_value))
				continue;
			...
			if (GET_ITEM_TYPE(obj) == ITEM_TRAP && obj->o.od.value[1] == -1) {
				if (trap_roll < (af->a.hidden.hidden_value - 15)) {
					if (obj->o.od.value[0] == 0)
						act(
								"You reveal $p, but too late you you accidentally triggered the trap in your search. Thankfully, it appears the trap was either not armed or a dud.",
								false, ch, obj, 0, TO_CHAR | _ACT_FORMAT);
					else {
						act(
								"You reveal $p, but too late you realise you have accidentally triggered the trap in your search...",
								false, ch, obj, 0, TO_CHAR | _ACT_FORMAT);
...
void do_look(CHAR_DATA * ch, char *argument, int cmd) {
	...
	const char *e_dirs[] = { "the north", "the east", "the south", "the west",
			"above", "below" };
	...
	if (strcasecmp(arg1, "arena") == STR_MATCH
			&& (ch->in_room == GRUNGE_VIP_VIEW
					|| ch->in_room == GRUNGE_PUBLIC_VIEW)) {
		grunge_arena__do_look(ch, argument, 0);
		return;
	} else if (strcasecmp(arg1, "arena") == STR_MATCH
			&& ch->in_room == GRUNGE_PRIVATE_VIEW) {
	...
	if ((strcasecmp(arg1, "window") == STR_MATCH && ch->in_room == OOC_LOUNGE)
			|| (!strn_cmp(arg1, "window", 6)
					&& strcasecmp(ch->room->name, PREGAME_ROOM_NAME)
							== STR_MATCH)) {
	...
	if (*arg1 && strcasecmp(arg1, "in") != STR_MATCH
		...
		if ((af = get_affect(ch, AFFECT_SHADOW))
				&& af->a.shadow.edge
						== dir && IS_SET (exit->exit_info, EX_ISDOOR)
	...
	if (strcasecmp(arg1, "in") == STR_MATCH) {
		...
		if (!(obj = get_obj_in_dark(ch, arg1, ch->right_hand)) && !(obj =
				get_obj_in_dark(ch, arg1, ch->left_hand)) && !(obj =
				get_obj_in_dark(ch, arg1, ch->equip)) && !(obj =
				get_obj_in_list_vis(ch, arg1, ch->room->contents))) {
		...
		if (GET_ITEM_TYPE (obj) == ITEM_DRINKCON) {
			...
			if (!obj->contains)
				...
				sprintf(buf, "$p is %sfull of %s.", fullness[temp],
			...
			if (obj->o.light.hours) {
				sprintf(buf, "$p looks like it will last about %d more hour%s.",
						obj->o.light.hours,
	...
	/* LOOK AT SOMETHING */
		...
		if (strcasecmp(arg1, "hands") == STR_MATCH
				|| strcasecmp(arg1, "hand") == STR_MATCH) {
			eq = false;
			room = false;
			argument = one_argument(argument, arg1);
		} else if (strcasecmp(arg1, "eq") == STR_MATCH
				|| strcasecmp(arg1, "equip") == STR_MATCH
				|| strcasecmp(arg1, "equipment") == STR_MATCH) {
			hands = false;
			room = false;
			argument = one_argument(argument, arg1);
		...
		if ((tch = get_char_room_vis(ch, arg1))) {
			...
			if (is_abbrev(arg1, "group")) {
				if (!do_group_size(tch)) {
					act("$N isn't in a group.", false, ch, 0, tch,
							TO_CHAR | _ACT_FORMAT);
					return;
				}
				...
				for (tch = top_leader->room->people; tch;
			...
			if (*arg1 && hands
					&& ((obj = get_obj_in_list_vis(tch, arg1, tch->right_hand))
							|| (obj = get_obj_in_list_vis(tch, arg1,
									tch->left_hand)))) {
		...
		if (!(obj = (hands ? get_obj_in_dark(ch, arg1, ch->right_hand) : NULL))
				&& !(obj = (
						hands ? get_obj_in_dark(ch, arg1, ch->left_hand) : NULL))
				&& !(obj = (eq ? get_obj_in_dark(ch, arg1, ch->equip) : NULL)))
			obj = (room ?
					get_obj_in_list_vis(ch, arg1, ch->room->contents) : NULL);

		/** BOARDS **/
		if (obj && obj->obj_flags.type_flag == ITEM_BOARD) {

			if (obj->clan_data && !(is_clan_member(ch, obj->clan_data->name))) {
				show_obj_to_char(obj, ch, 15);
				send_to_char(
						"\nYou can not read these reports, but you can write one.",
						ch);
				return;
			}
		...
		if (IS_MORTAL(ch)) {
			...
			sprintf(buf, "\n#6%s#0 #2[%d: %s#6%s#2]#0", ch->room->name,
					ch->room->vnum, bitbuf,
		...
		if (ch->vehicle && is_he_somewhere(ch->vehicle)
		...
		if (!IS_SET(ch->plr_flags, BRIEF_MODE)) {
			...
			for (dir = 0; dir <= LAST_DIR; dir++) {
				...
				if (!EXIT (ch, dir)->exit_info
						|| (dir > 5 && (obj = find_dwelling_obj(ch->room->vnum))
		...
		if (ch->room->scent) {
			...
			if (scent_count == 3) {
				sprintf(stink_buf,
						"#6You detect %s of %s, %s of %s and %s of %s in the air.#0",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref),
						scent_tier[scent_strength(scent_three)],
						scent_lookup(scent_three->scent_ref));
			} else if (scent_count == 2) {
				sprintf(stink_buf,
						"#6You detect %s of %s and %s of %s in the air.#0",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref));
			} else if (scent_count == 1) {
				sprintf(stink_buf, "#6You detect %s of %s in the air.#0",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref));
			}
		}
		...
		if (!IS_SET(ch->room->room_flags,
				INDOORS) && ch->room->sector_type != SECT_UNDERWATER) {
			*buf = '\0';
			if (!contents)
				sprintf(buf, "\n");
...
int read_virtual_message(CHAR_DATA * ch, char *name, char *argument) {
	...
	*name = toupper(*name);
	...
	if (!IS_MORTAL(ch)) {
		oss
				<< "#6Context:#0  http://www.middle-earth.us/staffportal.php?context="
				<< message->nTimestamp << "&db=all##" << message->nTimestamp
				<< "\n";
	}
...
void do_read(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!isdigit(*buf)) {
		...
		if (!((board_obj = get_obj_in_dark(ch, buf, ch->right_hand))
				&& (!USES_BOOK_CODE(board_obj)
			...
			if (IS_MORTAL (ch) || !read_pc_message(ch, buf, argument)) {
				...
				if ((IS_MORTAL (ch) && (strncmp(buf, "news", 4) != 0))
						|| !read_virtual_message(ch, buf, argument)) {
		...
		for (board_obj = ch->room->contents; board_obj;
...
void do_exits(CHAR_DATA * ch, char *argument, int cmd) {
	...
	char *exits[] = { "North", "East ", "South", "West ", "Up   ", "Down ",
			...
			"Southwest", "UpNorth", "UpEast", "UpSouth", "UpWest",
			"UpNortheast", "UpNorthwest", "UpSoutheast", "UpSouthwest",
			"DownNorth", "DownEast", "DownSouth", "DownWest", "DownNortheast",
			"DownNorthwest", "DownSoutheast", "DownSouthwest" };
	...
	*s_buf = '\0';
	...
	for (dir = 0; dir <= LAST_DIR; dir++) {
		...
		if (IS_SET(exit->exit_info,
				EX_ISDOOR) || IS_SET (exit->exit_info, EX_ISGATE)) {
			...
			&& !IS_SET (ch->affected_by, AFF_INFRAVIS))
				...
				sprintf(strExit + strlen(strExit), "%s - %s (Open) - %s",
						exits[dir], exit->keyword,
						vnum_to_room(exit->to_room)->name);
		}
			...
			&& !IS_SET (ch->affected_by, AFF_INFRAVIS))
				...
				sprintf(strExit + strlen(strExit), "%s - %s", exits[dir],
						vnum_to_room(exit->to_room)->name);
		}
...
int get_comestible_range(int num)  // Thirst only?
		{
	if (num <= 0)
		return 0;
...
int get_hunger_comestible_range(int num) // Changing to use MIN_CALORIE and MAX_CALORIE global values 0211141844 -Nimrod
		{
...
void hunger_thirst_process(CHAR_DATA * ch) {
	...
	*update_buf = '\0';
	...
	if (get_hunger_comestible_range(ch->hunger)
		...
		if (!(time_info.hour % 2))
			sprintf(update_buf + strlen(update_buf),
					"%s, slowly wasting away from hunger",
					verbal_hunger[get_hunger_comestible_range(ch->hunger)]);
	}
	...
	} else if (ch->hunger >= MIN_CALORIES * .75
			&& ch->hunger <= MIN_CALORIES * .5) {
		if ((af = get_affect(ch, MAGIC_STARVE_ONE)))
			remove_affect_type(ch, MAGIC_STARVE_ONE);
	...
	} else if (ch->hunger >= MIN_CALORIES * .5
			&& ch->hunger <= MIN_CALORIES * .25) {
		if ((af = get_affect(ch, MAGIC_STARVE_TWO)))
			remove_affect_type(ch, MAGIC_STARVE_TWO);
...
void do_info(CHAR_DATA * ch, char *argument, int cmd) {
	...
	sprintf(buf,
			"By virtue of your equipment, you are #2%s#0 and your movement #2%s#0 hindered.\n",
			(z >= 4 ? "extremely well armoured" : z == 3 ? "heavily armoured" :
				z == 2 ? "well armoured" :
				z == 1 ? "lightly armoured" : "unarmoured"),

			(x == 4 ? "rather" : x == 3 ? "noticably" : x == 2 ? "lightly" :
				x == 1 ? "faintly" : "not at all"));
			...
			for (af = ch->hour_affects; af; af = af->next) {
				if (af->type == AFFECT_COVER) {
					...
					if (j == 1)
						sprintf(buf2, " %s",
								(af->a.cover.direction == 6 ?
										"area" : dirs[af->a.cover.direction]));
					else if (j == covered)
						sprintf(buf2 + strlen(buf2), " and %s",
								(af->a.cover.direction == 6 ?
										"area" : dirs[af->a.cover.direction]));
					else
		...
		if (ch->fighting && ch->fighting->short_descr) {
			...
			for (rch = ch->room->people; rch; rch = rch->next_in_room) {
				if (rch->fighting == ch && rch != ch->fighting)
					act("You are also being attacked by $N.", false, ch, 0, rch,
							TO_CHAR);
			}
	...
	for (rch = ch->room->people; rch; rch = rch->next_in_room) {
		if ((af = get_affect(rch, MAGIC_GUARD))
	...
	sprintf(buf,
			"You add overwatch for targets #2%s#0, and take aim at targets that are #2%s#0 to hit.",
			(ch->over_target == 0 ? "manually" :
				ch->over_target == 1 ? "as directed" :
				ch->over_target == 2 ?
						"that shoot at you" : "that shoot at your group"),
			(ch->over_thresh == 0 ? "easy" :
				ch->over_thresh == 1 ? "moderate" : "any chance"));
	act(buf, false, ch, 0, 0, TO_CHAR);

	if (IS_SET(ch->plr_flags, AUTO_FIRE)) {
		send_to_char(
				"You automatically fire at targets you are aiming at it they try to move out of range.\n",
				ch);
	}
...
void print_score_mote(CHAR_DATA* ch, const char* const field,
		const char* index) {
	char buf[MAX_STRING_LENGTH] = { '\0' };
	if (index) {
...
void do_score(CHAR_DATA * ch, char *argument, int cmd) {
	...
	static char *verbal_stats[] = { "horrible", "bad", "poor", "average",
			"good", "great", "peak", "superhuman", "legendary", "epic" };
	...
	if (!IS_SET(ch->flags, FLAG_GUEST))
		sprintf(buf, "#2%s, a %d year old %s %s:#0\n", GET_NAME(ch),
				GET_AGE(ch), lookup_race_variable(ch->race,
				RACE_NAME), sex_types[ch->sex]);
	...
	if (ch->descr() && ch->descr()->acct
			&& ch->descr()->acct->get_last_nominate() > 0) {
	...
	sprintf(buf, "You are #2%s#0, and #2%s#0.\n",
			...
			MIN_CALORIES ?
					...
					"full",
			ch->thirst >= 0 ?
					verbal_thirst[get_comestible_range(ch->thirst)] :
					"quenched");
	send_to_char(buf, ch);
	...
	if (!IS_SET(ch->flags, FLAG_GUEST) && !IS_MORTAL(ch)) {
		if (birth_date.year > 0)
			sprintf(buf, "You were born on #2%s#0, #2%d#0.\n",
					short_time_string(birth_date.day, birth_date.month),
					birth_date.year); // can use call to suffix(birth_date.day) to show 'st', 'th', 'nd'
		else
			sprintf(buf, "You were born on #2%s#0, many millenia past.\n",
					short_time_string(birth_date.day, birth_date.month));
		send_to_char(buf, ch);
	}

	playing_time = real_time_passed(time(0) - ch->time.logon + ch->time.played,
			0);
		...
		if (!ch->hour_affects)
			...
			for (af = ch->hour_affects; af; af = af->next) {
				...
				if (af->type >= JOB_1 && af->type <= JOB_3) {
					...
					i = time_info.year * 12 * 30 + time_info.month * 30
							+ time_info.day;
					i = af->a.job.pay_date - i;

					vobj = vtoo(af->a.job.object_vnum);

					sprintf(buf, "   Job %d:  %d of %d days until payday\n",
							af->type - JOB_1 + 1, i, af->a.job.days);

					sprintf(buf + strlen(buf), "             %d x %s",
							af->a.job.count,
				...
				if (af->type == MAGIC_CRAFT_DELAY || AFFECT_UPGRADE_DELAY
						|| MAGIC_CRAFT_BRANCH_STOP)
					continue;
				...
				sprintf(buf, "   %s affects you by %d for %d hours.\n",
						p == NULL ? "Unknown" : p, af->a.spell.modifier,
						af->a.spell.duration);
				send_to_char(buf, ch);
			}
		}
	...
	if (IS_MORTAL(ch)) {
		...
		for (i = 1; i <= LAST_SKILL; i++) {
			...
			if ((af = get_affect(ch, MAGIC_FLAG_FOCUS + i))) {
				sprintf(buf2, "#2%12s:#0#6%s#0 ", skills[i],
						skill_lev2(who->skills[i]));
				while (strlen(buf2) < 35)
					strcat(buf2, " ");
			} else if ((af = get_affect(ch, MAGIC_FLAG_IGNORE + i))) {
				sprintf(buf2, "#2%12s:#0#5%s#0 ", skills[i],
						skill_lev3(who->skills[i]));

				while (strlen(buf2) < 35)
					strcat(buf2, " ");
			} else {
				sprintf(buf2, "%s%12s:#0 %s ",
						(ch->skills[i] == calc_lookup(ch, REG_CAP, i) ?
								"#3" : "#2"), skills[i],
						skill_lev(who->skills[i]));
		...
		for (i = 1; i <= LAST_SKILL; i++) {
			if (real_skill(who, i)) {
				sprintf(buf + strlen(buf), "#2%8.8s#0[%03d/%03d] ", skills[i],
						who->skills[i], calc_lookup(who, REG_CAP, i));
				if (!(j % 4))
					strcat(buf, "\n");
				j++;
			}
	...
	*buf = '\0';
	...
	if (ch->talents <= 0) {
		...
		if (IS_SET(ch->talents, TAL_STR1))
			sprintf(buf + strlen(buf),
					"      #2Upgraded Attribute:#0 Strength\n");
		if (IS_SET(ch->talents, TAL_DEX1))
			sprintf(buf + strlen(buf),
					"      #2Upgraded Attribute:#0 Dexterity\n");
		if (IS_SET(ch->talents, TAL_CON1))
			sprintf(buf + strlen(buf),
					"      #2Upgraded Attribute:#0 Constitution\n");
...
int number_of_helpfiles_available(int player_level) {
	// Build a query to get the number of helpfiles for that level
	int number_of_helpfiles = 0;
	std::ostringstream query_stream;
	query_stream << "SELECT COUNT(*) AS num_helpfiles " << "FROM helpfiles "
			<< "WHERE required_level <= " << player_level;
	...
	if ((mysql_safe_query((char *) query.c_str())) == 0) {
		...
		if ((result = mysql_store_result(database)) != 0) {
			...
			std::string error_message = "number_of_helpfiles_available: "
...
std::string output_categories_available(int player_level) {
	std::string category_list;

	// Query a list of categories from the helpfiles table
	std::ostringstream query_stream;
	query_stream << "SELECT category " << "FROM helpfiles "
			<< "WHERE required_level <= " << player_level << ' '
	...
	if ((mysql_safe_query((char *) query.c_str())) == 0) {
		...
		if ((result = mysql_store_result(database)) != 0) {
			...
			std::string error_message = "output_categories_available: "
...
void do_help(CHAR_DATA * ch, char *argument, int cmd) // If cmd is greater than 99 the help function will not log a missing helpfile, or spit out partial matches. 0212142020 -Nimrod
		{
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	char buf[MAX_STRING_LENGTH] = { '\0' };
...
int show_where_char(const CHAR_DATA * ch, int indent) {
	char buf[MAX_STRING_LENGTH] = { '\0' };
...
int show_where_obj(OBJ_DATA * obj, int indent) {
	...
	sprintf(buf + strlen(buf), "%s (%d)", obj->short_description,
			obj->nVirtual);
	...
	if (!*name || check_aura || check_clan) {
		if (IS_MORTAL(ch)) {
			...
			for (x = 0; x < nDescriptors; x++) {
				...
				bIsInWater = (ch_room
						&& (ch_room->sector_type == SECT_RIVER
				...
				for (d = descriptor_list; d; d = d->next) // iterate through characters to find out if they are in room arrRoom[x]
					...
					if (d->character && (d->connected == CON_PLYNG)
						...
						if ((d->original
							...
							strcpy(strFmtName, "#6");
						} else if (IS_SET(d->character->plr_flags,
								NEW_PLAYER_TAG)) {
							strcpy(strFmtName, "#2");
						...
						default: {
							...
							else if (IS_SET (i->act, PLR_QUIET) && !IS_NPC(i))
								chrState = 'e';
						...
						sprintf(strFmtStatus, "%20.20s",
								i->status_str ? i->status_str : "(none)");

						/* Put the room together */
						nMaxFmtRoomLen = (WHERE_LINE_LEN + 6
								+ ((strlen(strFmtAnim) > 0) ? 4 : 0))
								- strlen(strFmtName) - strlen(strFmtAnim)
								- strlen(strFmtStatus);
						...
						sprintf(buf + strlen(buf), "%s#0 #%c%c#0 %s %s#0%s#0\n",
								strFmtName,
								(chrState <= 'Z' && chrState >= 'A') ?
										'1' :
										((chrState <= 'z' && chrState >= 'a') ?
												'3' : '0'), chrState,
								strFmtStatus, strFmtRoom, strFmtAnim);
					}
				}
		...
		for (k = object_list; k; k = k->next) {
			...
			if (isname(name, k->name) && CAN_SEE_OBJ(ch, k)) {
				if (k->carried_by && !k->carried_by->deleted
					...
					sprintf(buf + strlen(buf), "#2%-20.20s#0- #2[%5d]#0 #6%s#0",
	...
	*s_buf = '\0';
	...
	for (d = descriptor_list; d; d = d->next) {
		...
		if (IS_MORTAL(d->character)) {
			if (d->character->pc && !d->character->pc->level
					&& d->character->pc->create_state
							== 2 && !IS_SET (d->character->flags, FLAG_GUEST))
				mortals++;
...
void do_scan(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!*buf) {
		...
		for (dir = 0; dir <= LAST_DIR; dir++) {
			if ((exit = EXIT(ch, dir)) && vnum_to_room(exit->to_room)
		...
		if (cmd != 1) {
			if (IS_OUTSIDE(ch))
				...
				act("You peer into the distance . . .", false, ch, 0, 0,
						TO_CHAR);
		}
		area_scan(ch, cmd);
		return;
...
void area_scan(CHAR_DATA * ch, int cmd) {
	...
	for (dir = 0; dir <= LAST_DIR; dir++) {
		...
		if ((exit = EXIT(ch, dir))
				/*&& !IS_SET (exit->exit_info, EX_ISDOOR)*/
				&& !IS_SET(exit->exit_info,
						EX_CLOSED) && (next_room = vnum_to_room (exit->to_room))
			...
			sprintf(buf, "%s, #6%s#0, you see...\n", dirs[dir],
					IS_LIGHT (next_room) ?
							next_room->name : "#4where it is dark#0");
			*buf = toupper(*buf);
			send_to_char(buf, ch);
...
get_diagonal_scan_room(CHAR_DATA * ch, int nDirOne, int nDirTwo) {
	...
	if ((ptrTmpExit = EXIT(ch, nDirOne))
			&& ((!IS_SET(ptrTmpExit->exit_info, EX_ISDOOR)) && (ptrTmpRoom =
					vnum_to_room(ptrTmpExit->to_room))
 ...
 *
 */
const char *diag_dirs[] = { "northeast", "southeast", "southwest", "northwest",
		"\n" };
	...
	*buf2 = '\0';
	...
	if (ch->left_hand) {
		if (GET_ITEM_TYPE (ch->left_hand) == ITEM_MONEY) {
			...
			for (tobj = ch->left_hand->contains; tobj; tobj =
					tobj->next_content) {
				if (GET_ITEM_TYPE (tobj) == ITEM_MONEY) {
					money += (int) tobj->farthings * tobj->count;
					sprintf(buf2 + strlen(buf2),
...
void post_track_response(DESCRIPTOR_DATA * d) {
	...
	*buf = '\0';
	...
	message = load_message(d->character->delay_who, 5,
...
void do_erase(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!isdigit(*buf)) {
		if (!(obj = get_obj_in_list_vis(ch, buf, ch->room->contents)) && !(obj =
				get_obj_in_list_vis(ch, buf, ch->right_hand)) && !(obj =
				get_obj_in_list_vis(ch, buf, ch->left_hand))) {
			if (IS_MORTAL(ch)
		...
		if (obj->obj_flags.type_flag != ITEM_BOARD) {
			act("$p, the first board here, isn't a real board.", false, ch, obj,
					0, TO_CHAR);
			return;
		}
	...
	if (!GET_TRUST(ch) && strcasecmp(GET_NAME(ch), message->poster) != STR_MATCH) {
...
int write_virtual_board(CHAR_DATA * ch, char *name, char *argument) {
	...
	*name = toupper(*name);
	...
	ch->descr()->pending_message = (MESSAGE_DATA *) alloc(sizeof(MESSAGE_DATA),
			1);
...
void show_unread_news_messages(CHAR_DATA * ch) {
	...
	mysql_safe_query(
			"SELECT post_number, subject, date_posted FROM virtual_boards WHERE timestamp >= %d AND board_name=\"news\" ORDER BY post_number desc",
			(int) ch->pc->last_logon);
	result = mysql_store_result(database);
...
void add_message(int new_message, const char *name, int nVirtual,
		const char *poster, char *date, char *subject, char *info,
	...
	*date_buf = '\0';
	...
	if (time_info.holiday > 0)
		sprintf(date_buf, "%s, %d AD", holiday_short_names[time_info.holiday],
				time_info.year);
	else
		sprintf(date_buf, "%s/%d AD", short_time_string(day, time_info.month),
				time_info.year);
...
load_message(char *msg_name, int pc_message, int msg_number) {
	...
	*date_buf = '\0';
	...
	if (!*date_buf) {
		if (time_info.holiday > 0)
			sprintf(date_buf, "%s, %d AD",
					holiday_short_names[time_info.holiday], time_info.year);
		else
			sprintf(date_buf, "%s, %d AD",
					short_time_string(day, time_info.month), time_info.year);
	}
...
void do_notify(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (is_clan_member_player(ch, buf)) {
		...
		sprintf(buf2, "#3[%s (%s) is online.  Use NOTIFY to reply in kind.]#0",
				char_short(ch),
				(((clan = get_clandef_long(buf)) != NULL) ? clan->literal : buf));
			...
			if (result && mysql_num_rows(result) > 0
				...
				while ((row = mysql_fetch_row(result))) {
					if ((tch = get_pc(row[0]))) {
						...
						get_affect(tch, MAGIC_NOTIFY)->a.spell.t =
								(long int) ch;
						notify = true;
					}
				}
...
void post_writing(DESCRIPTOR_DATA * d) {
	...
	mod = (skill_level(ch, ch->writes, 0) * 0.50)
...
void do_flip(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!(obj = get_obj_in_list_vis(ch, buf, ch->right_hand)) && !(obj =
			get_obj_in_list_vis(ch, buf, ch->left_hand)) && !(obj =
			get_obj_in_list_vis(ch, buf, ch->room->contents))) {
	...
	if (IS_ELECTRIC(obj))
		if (atoi(argument) == 0)
			sprintf(buf,
					"You navigate through #2%s#0's interface until you locate the file block index.",
					obj->short_description);
		else
			sprintf(buf,
					"You navigate through #2%s#0's interface until you locate file block %d.",
...
void do_tear(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if ( GET_ITEM_TYPE (obj) == ITEM_BOOK) {
		...
		sprintf(buf, "%s#0 carefully tears a page from #2%s#0.", char_short(ch),
				obj->short_description);
		sprintf(buffer, "#5%s", CAP(buf));
...
void do_scribe(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (ch->skills[SKILL_EDUCATION] < 10)
		send_to_char(
				"OOC: Remember to RP the education level of your PC at all times. With"
	...
	if ( GET_PAGE_OVAL( obj ) == -1 && obj->writing) {
		...
		if (IS_ELECTRIC(obj)) {
			...
			if (!obj->writing || !obj->o.od.value[page_oval]) {
				send_to_char(
						"The device's memory unit appears to be corrupt.\n",
						ch);
				return;
			}
	...
	if ( GET_ITEM_TYPE( obj ) == ITEM_E_BOOK)
		sprintf(buffer, "#5%s#0 begins typing into #2%s#0.", char_short(ch),
				obj->short_description);
	else
...
void do_ticket(CHAR_DATA * ch, char *argument, int cmd) {
	char buf[AVG_STRING_LENGTH];
	char buf2[12];
	char f_tick[12];
	char l_tick[12];
	...
	if (!strcmp(buf, "browse")) {
		...
		if (first_tick >= last_tick) {
			send_to_char(
					"The last number must be larger than the first number.\n",
					ch);
			return;
		}
...
//Look for the MOB
		if (sscanf(buf, "%d %s", &nVirtual, hookup) != 2) {
			fclose(fp);
			system_log("The ticket system is broken, read_ticket() - mob",
					true);
		...
		if (mob) {
			sprintf(buf2 + strlen(buf2),
					"Vnum: %d \nNamed: %s \nClans: %s \nOwner: %s \nStabled at: %s (%d) \n",
					mob->mob->vnum, mob->name, mob->clans, mob->mob->owner,
					vnum_to_room(mob->in_room)->name, mob->in_room);
...
//        char		buf2 [MAX_STRING_LENGTH] = { '\0' };
	...
	if (obj->obj_flags.type_flag == ITEM_FOOD
		...
		if (ch->skills[SKILL_LORE] >= obj->o.od.value[4]
			...
			case 1:
				sprintf(buffer,
						"You determine that it is safe, edible food.\n");
				break;
			case 2:
				sprintf(buffer,
						"You determine that it will be safe to eat once properly cooked or prepared.\n");
				break;
			case 3:
				sprintf(buffer,
						"You determine that it is dangerous to eat, but can be prepared in other ways.\n");
				break;
			case 4:
				sprintf(buffer,
						"You determine that it is not edible but may have other uses.\n");
				break;
			default:
	...
	*buffer = '\0';
	...
	if (IS_SET(obj->obj_flags.wear_flags, ITEM_TAKE)) {
		...
		/*** Calculate estimate ***/
		guess_weight = (int) ((obj->obj_flags.weight + obj->contained_wt)
				/ variance);
		...
		{
			snprintf(buffer, MAX_STRING_LENGTH,
					"\nYou would guess that this item weighs about %d pounds.",
					guess_weight);
		}
	...
	*buffer = '\0';
	...
	if ( GET_ITEM_TYPE (obj) == ITEM_DRINKCON) {
		if (!obj->contains) {
			snprintf(buffer, MAX_STRING_LENGTH,
					"\nYou can see that %s is empty.", obj->short_description);
		} else {
			if (obj->o.drinks.capacity) {
				temp = (obj->contains->count * 3) / obj->o.drinks.capacity;
			} else {
	...
	if (GET_ITEM_TYPE (obj) == ITEM_FOOD) {
		if (obj->morphTime) {
			...
			if (days > 1) {
				sprintf(buffer,
						"\nYou notice that %s appears fresh with no sign of decay.",
						obj->short_description);
			} else if (hours > 12) {
				sprintf(buffer,
				...
				sprintf(buffer,
						"\nWhile still in good condition, %s no longer appears fresh.",
						obj->short_description);
			} else {
				sprintf(buffer,
...
void do_check(CHAR_DATA *ch, char *argument, int cmd) {
	...
	if (!*arg1) {
		send_to_char(
				"What keyword do you want to check?\nTry 'check usage' for help.\n",
				ch);
		return;
	}
	...
	if ((!strn_cmp(arg1, "obj", strlen(arg1)))
			|| (!strn_cmp(arg1, "object", strlen(arg1)))) {
		argument = one_argument(argument, arg1);
		...
		if (!strn_cmp(arg1, "list", strlen(arg1))) {
			...
			int i = 0;
			for (obj = ch->room->contents; obj != NULL; obj =
					obj->next_content) {
	...
	if (!strncmp(arg1, "list", strlen(arg1))) {
		...
		if (*arg2 && (!strncmp(arg2, "in", strlen(arg2)))) {
			...
			if (!(container = get_obj_in_dark(ch, arg2, ch->right_hand))
					...
					&& !(container = get_obj_in_list_vis(ch, arg2,
							ch->room->contents))) {
		...
		//check list <keyword> - for mobs and objects in the room.
			...
			int i = 0;
			for (tch = (get_char_room_vis(ch, arg1)); tch;
					tch = tch->next_in_room) {
				if (isname(arg1, tch->name) && CAN_SEE(ch, tch)) {
					i++;
					sprintf(buf, "%i.%s: #5%s#0 \n", i, arg1, char_short(tch));
...
void do_smell(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!*buf) {
		room = ch->room;
		if (!room->scent) {
			act("You sniff the air, but don't detect any particular scent.",
					false, ch, 0, 0, TO_CHAR | _ACT_FORMAT);
			...
			if (scent_count == 3) {
				sprintf(buf2,
						"You sniff the air, and detect %s of %s, %s of %s and %s of %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref),
				...
				sprintf(buf2,
						"You sniff the air, and detect %s of %s and %s of %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref));
			} else if (scent_count == 1) {
				sprintf(buf2, "You sniff the air, and detect %s of %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref));
			} else {
				sprintf(buf2,
						"You sniff the air, but don't detect any particular scent.");
			}
	...
	if (!(tch = get_char_room_vis(ch, buf))) {
		if (!(obj = get_obj_in_dark(ch, buf, ch->right_hand)) && !(obj =
				get_obj_in_dark(ch, buf, ch->left_hand)) && !(obj =
				get_obj_in_dark(ch, buf, ch->equip)) && !(obj =
				get_obj_in_list_vis(ch, buf, ch->room->contents))) {
			send_to_char("You don't see that here.\n", ch);
			return;
		} else if (!obj->scent) {
			act("You sniff $p, but don't detect any particular scent.", false,
					ch, obj, 0, TO_CHAR | _ACT_FORMAT);
			...
			if (scent_count == 3) {
				sprintf(buf2,
						"You sniff $p, and detect %s of %s, %s of %s and %s of %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref),
				...
				sprintf(buf2,
						"You sniff $p, and detect %s of %s and %s of %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref),
						scent_tier[scent_strength(scent_two)],
						scent_lookup(scent_two->scent_ref));
			} else if (scent_count == 1) {
				sprintf(buf2, "You sniff $p, and detect %s of %s. ",
						scent_tier[scent_strength(scent_one)],
						scent_lookup(scent_one->scent_ref));
			} else {
				sprintf(buf2,
						"You sniff $p, but don't detect any particular scent.");
			}
		...
		act("Why don't you just examine yourself instead?", false, ch, 0, 0,
				TO_CHAR | _ACT_FORMAT);
		return;
	} else if (!tch->scent) {
		act("You sniff $N, but don't detect any particular scent.", false, ch,
				0, tch, TO_CHAR | _ACT_FORMAT);
		...
		if (scent_count == 3) {
			sprintf(buf2,
					"You sniff $N, and detect %s of %s, %s of %s and %s of %s. ",
					scent_tier[scent_strength(scent_one)],
					scent_lookup(scent_one->scent_ref),
					scent_tier[scent_strength(scent_two)],
					scent_lookup(scent_two->scent_ref),
			...
			sprintf(buf2, "You sniff $N, and detect %s of %s and %s of %s. ",
					scent_tier[scent_strength(scent_one)],
					scent_lookup(scent_one->scent_ref),
					scent_tier[scent_strength(scent_two)],
					scent_lookup(scent_two->scent_ref));
		} else if (scent_count == 1) {
			sprintf(buf2, "You sniff $N, and detect %s of %s. ",
					scent_tier[scent_strength(scent_one)],
					scent_lookup(scent_one->scent_ref));
		} else {
			sprintf(buf2,
					"You sniff $N, but don't detect any particular scent.");
		}
