...
bool is_outdoors(ROOM_DATA *room) {
	...
	if (room->sector_type == SECT_BOARDWALK || room->sector_type == SECT_CITY
			|| room->sector_type == SECT_ROAD || room->sector_type == SECT_TRAIL
			...
			|| room->sector_type == SECT_SWAMP || room->sector_type == SECT_DOCK
			...
			|| room->sector_type == SECT_RIVER || room->sector_type == SECT_REEF
...
const int movement_loss[] = { 1, /* Inside */
2, /* MoonPlain */
4, /* MoonHill */
6, /* MoonMountain */
6, /* MoonSand */
...
const int terrain_walk_time[] = { 1, /* Inside */
1, /* MoonPlain */
3, /* MoonHill */
5, /* MoonMountain */
5, /* MoonSand */
...
const int terrain_penalty[23][2] = { { 20, 20 }, // inside
		{ 20, 20 }, // city
		{ 20, 20 }, // road
		{ 0, 10 }, // trail
		{ 0, 20 }, // field
...
const char *leg_woundlocs[] = { "groin", "rthigh", "lthigh", "rknee", "lknee",
		"rcalf", "lcalf", "rfoot", "lfoot", "\n" };

const char* const relative_dirs[] = { "northern", "eastern", "southern",
		"western", "above", "below", "outside", "inside", "northeastern",
		"northwestern", "southeastern", "southwestern",

		"upper northern", "upper eastern", "upper southern", "upper western",
		"upper northeastern", "upper northwestern", "upper southeastern",
		"upper southwestern", "lower northern", "lower eastern",
		"lower southern", "lower western", "lower northeastern",
		"lower northwestern", "lower southeastern", "lower southwestern"

};

const char* const dirs[] = { "north", "east", "south", "west", "up", "down",
		"outside", "inside", "northeast", "northwest", "southeast", "southwest",
		"upnorth", "upeast", "upsouth", "upwest", "upnortheast", "upnorthwest",
		"upsoutheast", "upsouthwest", "downnorth", "downeast", "downsouth",
		"downwest", "downnortheast", "downnorthwest", "downsoutheast",
		"downsouthwest", "\n" };

// short_dirs MUST match dirs[] above -Nimrod
const char* const short_dirs[] = { "n", "e", "s", "w", "u", "d", "o", "i", "ne",
		"nw", "se", "sw", "un", "ue", "us", "uw", "une", "unw", "use", "usw",
		"dn", "de", "ds", "dw", "dne", "dnw", "dse", "dsw", "\n" };

const int rev_dir[] = { 2, 3, 0, 1, 5, 4, 7, 6, 11, 10, 9, 8, 22, 23, 20, 21,
		27, 26, 25, 24, 14, 15, 12, 13, 19, 18, 17, 16 };

const char* const speeds[] = { "walk", /* 1.00 */
"trudge", /* 2.50 */
"pace", /* 1.60 */
"jog", /* 0.66 */
"run", /* 0.50 */
...
const char* const mount_speeds[] = { "trot", "walk", "slow trot", "canter",
		"race", "gallop", "immwalk", "swim", "float", "\n" };

const char* const mount_speeds_ing[] = { "trotting", "walking",
		"slowly trotting", "cantering", "racing", "galloping", "immwalking",
		"swimming", "floating", "\n" };

const char *direction[] = { "south", "west", "north", "east", "down", "up",
		"inside", "outside", "southwest", "southeast", "northwest", "northeast",
		"downsouth", "downwest", "downnorth", "downeast", "downsouthwest",
		"downsoutheast", "downnorthwest", "downnortheast", "upsouth", "upwest",
		"upnorth", "upeast", "upsouthwest", "upsoutheast", "upnorthwest",
		"upnortheast", "report_to_nimrod0257072314" };

const char *rev_d2[] = { "the south", "the west", "the north", "the east",
		"below", "above", "inside", "outside", "the southwest", "the southeast",
		"the northwest", "the northeast", "the downsouth", "the downwest",
		"the downnorth", "the downeast", "the downsouthwest",
		"the downsoutheast", "the downnorthwest", "the downortheast",
		"the upsouth", "the upwest", "the upnorth", "the upeast",
		"the upsouthwest", "the upsoutheast", "the upnorthwest",
		"the upnortheast", "report this to nimrod" };
...
const struct encumberance_info enc_tab[] =
		{ { 300, 0, 0, 0, 0, 1.00, "unencumbered" }, { 600, 5, 2, 0, 0, 1.00,
				"lightly encumbered" },
				{ 900, 10, 4, 5, 1, 0.95, "encumbered" }, { 1300, 15, 6, 10, 2,
						0.90, "heavily encumbered" }, { 2000, 25, 8, 15, 2,
						0.80, "critically encumbered" }, { 9999900, 25, 4, 15,
						2, 0.70, "immobile" }, };

const struct fatigue_data fatigue[] = { { 5, 0.65, "Completely Exhausted" }, {
		15, 0.70, "Exhausted" }, { 25, 0.75, "Extremely Tired" }, { 35, 0.80,
		"Tired" }, { 45, 0.85, "Somewhat Tired" }, { 55, 0.90, "Winded" }, { 75,
		0.95, "Somewhat Winded" }, { 90, 1.00, "Relatively Fresh" }, { 999,
		1.00, "Completely Rested" } };
...
bool skip_sneak_check(CHAR_DATA *ch) {
...
int ride_mount(CHAR_DATA *mount, CHAR_DATA *rider, int current, int target,
		int dir) {
	int mod = 0;
	int speed;
	int rskill;
	...
	if (number(1, SKILL_CEILING) > rskill + 40) {
		sprintf(buf,
				"As $n %s %s, you slide and fall from $s back in a particularly graceless display.",
				mount_speeds[speed], dirs[dir]);
		sprintf(buf2,
				"As you %s %s, you feel strangely bemused as $N falls from your back.",
				mount_speeds[speed], dirs[dir]);
		sprintf(buf3, "As $n %s %s, $N slowly slides and falls from $s back.",
				mount_speeds[speed], dirs[dir]);
	...
	if (check > mskill) {
		if (mod < 15) {
			sprintf(buf,
					"As $n %ss %s, $e trips and stumbles, legs scrabbling for a moment before regaining $s standing.",
					mount_speeds[speed], dirs[dir]);
			sprintf(buf2,
					"As you %ss %s, you suddenly trip and stumble, legs scrabbling for a moment before you regain your standing.",
					mount_speeds[speed], dirs[dir]);
			sprintf(buf3,
					"As $n %ss %s, $e trips and stumbles, legs scrabbling for a moment before regaining $s standing.",
					mount_speeds[speed], dirs[dir]);
			...
			sprintf(buf,
					...
					mount_speeds[speed], dirs[dir]);
			sprintf(buf2,
					"As you %ss %s, you stumble before falling to the ground.",
					mount_speeds[speed], dirs[dir]);
			sprintf(buf3,
					"As $n %ss %s, $e stumbles before falling to the ground.",
					mount_speeds[speed], dirs[dir]);
			...
			sprintf(buf,
					...
					mount_speeds[speed], dirs[dir]);
			sprintf(buf2,
					"As you attempt tos %s %s, you lose your footing and hit the ground hard.",
					mount_speeds[speed], dirs[dir]);
			sprintf(buf3,
					"$n attempts to %ss %s but loses $s footing and hits the ground hard.",
					mount_speeds[speed], dirs[dir]);
			...
			wound_to_char(mount, figure_location(mount, HITLOC_BODY),
					dice(1, 8), 3, 0, 0, 0);
			wound_to_char(mount, figure_location(mount, HITLOC_LOLEGS),
					dice(1, 8), 3, 0, 0, 0);
			wound_to_char(mount, figure_location(mount, HITLOC_LOLEGS),
					dice(1, 8), 3, 0, 0, 0);
		} else {
			sprintf(buf,
					...
					mount_speeds[speed], dirs[dir]);
			sprintf(buf2,
					"You attempt to %ss %s but trip and smash in to the ground.",
					mount_speeds[speed], dirs[dir]);
			sprintf(buf3,
					"$n attempts to %ss %s but trips and smashes in to the ground.",
					mount_speeds[speed], dirs[dir]);
			...
			wound_to_char(mount, figure_location(mount, HITLOC_BODY),
					dice(1, 12), 3, 0, 0, 0);
			wound_to_char(mount, figure_location(mount, HITLOC_HEAD),
					dice(1, 12), 3, 0, 0, 0);
			wound_to_char(mount, figure_location(mount, HITLOC_HEAD),
					dice(1, 12), 3, 0, 0, 0);
		}
		...
		if (mod < 0) {
			sprintf(buf + strlen(buf),
					" Despite being jolted and shaken in the commotion, you manage to stay atop $n.");
			sprintf(buf2 + strlen(buf2),
					" Despite the commotion you undergo, $N remains mounted upon you through it.");
			sprintf(buf3 + strlen(buf3),
					" Shaken and jolted, $N remains atop of $n.");
		} else if (mod < 15) {
			sprintf(buf + strlen(buf),
					" You fall slowly from $n's back, landing safely but abruptly on the ground.");
			sprintf(buf2 + strlen(buf2),
					" You feel $N slide slowly from your back.");
			sprintf(buf3 + strlen(buf3),
			...
			sprintf(buf + strlen(buf),
					" You are jolted forcefully from $n's back, landing with a painful thud.");
			sprintf(buf2 + strlen(buf2),
					" You feel $N slip from from your back.");
			sprintf(buf3 + strlen(buf3),
			...
			wound_to_char(rider, figure_location(rider, HITLOC_BODY),
					dice(1, 8), 3, 0, 0, 0);
			wound_to_char(rider, figure_location(rider, HITLOC_LOLEGS),
					dice(1, 8), 3, 0, 0, 0);
			wound_to_char(rider, figure_location(rider, HITLOC_HEAD),
					dice(1, 8), 3, 0, 0, 0);
			rider->mount = NULL;
			mount->mount = NULL;
		} else {
			sprintf(buf + strlen(buf),
					" You are flung forcefully from $s back, landing with a sickening crunch.");
			sprintf(buf2 + strlen(buf2),
					" You feel $N flung from from your back.");
			sprintf(buf3 + strlen(buf3),
			...
			wound_to_char(rider, figure_location(rider, HITLOC_HEAD),
					dice(1, 12), 3, 0, 0, 0);
			wound_to_char(rider, figure_location(rider, HITLOC_NECK),
					dice(1, 12), 3, 0, 0, 0);
			rider->mount = NULL;
			mount->mount = NULL;
		}
...
void do_party(CHAR_DATA * ch, char *argument, int cmd) {
...
int suffocated(CHAR_DATA *ch) {
	...
	if (!IS_MORTAL(ch) || number(0, 9)
			|| (IS_SET(ch->act, ACT_FLYING)
					&& !(IS_SET(ch->room->room_flags, NOAIR)))) {
		return 0;
	}
	...
	act("Thrashing sluggishly, $n struggles as they begin to choke.", false, ch,
...
// Free fall check.
int floating_check(CHAR_DATA * ch) {
...
void leave_tracks(CHAR_DATA * ch, int to_dir, int from_dir) {
	...
	if ((ch->room->sector_type == SECT_INSIDE
			|| ch->room->sector_type == SECT_OUTSIDE))
		return;
	...
	if (bleeding
			< 1&& lookup_race_int(ch->race, RACE_TRACKS) == RACE_TRACKS_SMALL) {
		return;
	}
...
void stop_mount_train(CHAR_DATA * stopper) {
	...
	if (stopper == leader)
		...
		act("You prevent the hitches led by $N from moving.", true, stopper, 0,
				leader, TO_CHAR);

	act("$N prevents your hitches from moving.", true, leader, 0, stopper,
			TO_CHAR);
	act("The hitches led by $n are stopped by $N.", false, leader, 0, stopper,
			TO_NOTVICT);
}
...
void outside_inside(CHAR_DATA *ch) {
	char w_phrase[50] = { '\0' };
	...
	*buf2 = '\0';
	...
	case PHASE_FULL_EARTH:
		sprintf(buf3 + strlen(buf3),
				" Against the backdrop of a star-scattered eternal space, the massive form of the Earth is now in full view, its marred surface reflecting an eerie bright white glow upon the moonscape.");
		break;
	case PHASE_GIBBOUS_WANING:
		sprintf(buf3 + strlen(buf3),
				" In the velvety, star-scattered night sky, the majestic Earth reflects an eerie white glow upon the surface of the Moon, though a thin sliver is overcome by darkness.");
		break;
	case PHASE_THREE_QUARTER:
		sprintf(buf3 + strlen(buf3),
				" The golden rays of the Sun begin to kiss the Moon's surface as its giant fiery form ascends over the horizon. A large portion of the majestic Earth is plunged into darkness with only a moderate crescent of silvery white remaining.");
		break;
	case PHASE_CRESCENT_WANING:
		sprintf(buf3 + strlen(buf3),
				" The Earth, now only a very thin sliver of silvery white, is plunged nearly entirely into darkness against an eternal backdrop of twinkling stars. The fiery giant that is the Sun now begins to mercilessly cast its intense heat and light upon the surface of the Moon.");
		break;
	case PHASE_NEW_EARTH:
		sprintf(buf3 + strlen(buf3),
				" The Sun has now reached its highest point in the sky, its omnipresent form casting blistering heat and intense light mercilessly onto the surface of the Moon. The majestic earth is now cast entirely in darkness, barely visible.");
		break;
	case PHASE_CRESCENT_WAXING:
		sprintf(buf3 + strlen(buf3),
				" The first signs of light have begun to creep over the surface of the Earth in the sky, giving it a silvery white glow in the form of a large sickle in the sky. The Sun's heat continues to beat mercilessly down upon the Moon, though its position in the sky has lowered.");
		break;
	case PHASE_FIRST_QUARTER:
		sprintf(buf3 + strlen(buf3),
				" The fiery giant that is the sun has now begun its descent across the Moon sky, casting the surroundings in a golden dusk. A quarter of the majestic Earth is now cast in a silvery reflective light against a backdrop of eternal space and stars.");
		break;
	default:
		sprintf(buf3 + strlen(buf3),
				" Against a backdrop of eternal space and glittering stars, the majestic Earth is now cast mostly in a silvery white, casting an eerie glow upon the surface of the cold Moon.");
		break;
...
void enter_room(QE_DATA * qe) {
	...
	const char *direction2[] = { "north", "east", "south", "west", "up", "down",
			"outside", "inside", "northeast", "northwest", "southeast",
			"southwest", "upnorth1", "upeast1", "upsouth1", "upwest1",
			"upnortheast1", "upnorthwest1", "upsoutheast1", "upsouthwest1",
			"downnorth1", "downeast1", "downsouth1", "downwest1",
			"downnortheast1", "downnorthwest1", "downsoutheast1",
			"downsouthwest1"

	};
	const char *dir_names[] = { "to the north", "to the east", "to the south",
	...
	if (!is_toll_paid(ch, qe->dir)) {
		...
		act("$n is stopped by $N.", true, ch, 0,
				toll_collector(ch->room, qe->dir), TO_NOTVICT);
		act("You stop $N for toll.", false, toll_collector(ch->room, qe->dir),
				0, ch, TO_CHAR);
		clear_moves(ch);
		if (qe->travel_str)
			mem_free(qe->travel_str);
	...
	if (ch->room->sector_type == SECT_FREEFALL
			&& IS_SET(ch->flags,
					FLAG_LEAVING) && IS_ENCUMBERED (ch) && IS_MORTAL (ch)
					) {
		send_to_char(
				"You float without difficulty despite your encumberance.\n",
				ch);
	}
	...
	for (dir = 0; dir <= LAST_DIR; dir++) {
		if (!(exit = EXIT(ch, dir))
				...
				|| IS_SET(exit->exit_info,
						EX_CLOSED) || GET_FLAG (ch, FLAG_WIZINVIS))
			continue;
		...
		for (tch = next_room->people; tch; tch = tch->next_in_room) {
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir] == waf->a.shadow.edge) {
					...
					if (qe->dir == waf->a.shadow.edge
							|| rev_dir[qe->dir] == waf->a.shadow.edge)
					...
					if (IS_SET(qe->flags, MF_SNEAK)
						...
						qe->flags &= ~MF_SNEAK;
						sprintf(buf,
								"To the %s, you see $n stealthily leave %sward.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								direction2[qe->dir]);
						...
						if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
							...
							sprintf(buf,
									"To the %s, you see #5a %s#0 led by $n leave %sward.\n",
									direction[rev_dir[waf->a.shadow.edge]],
									qe->group_str, direction2[qe->dir]);
							act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
						} else if (!(are_grouped(ch, tch)
								&& rev_dir[waf->a.shadow.edge] == qe->dir
								&& GET_FLAG(tch, FLAG_ENTERING))) {
							sprintf(buf,
									"To the %s, you see $n leave %sward.\n",
									direction[rev_dir[waf->a.shadow.edge]],
									direction2[qe->dir]);
							act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
						}
		...
		if (!(exit) || !(next_room = vnum_to_room(exit->to_room))
				|| IS_SET(exit->exit_info,
						EX_CLOSED) || IS_SET (qe->flags, MF_SNEAK) || GET_FLAG (ch, FLAG_WIZINVIS))
			continue;
			...
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir] == waf->a.shadow.edge) {
					...
					if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
						...
						sprintf(buf,
								"Far to the %s, you see #5a %s#0 led by $n leave %sward.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								qe->group_str, direction2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
					} else {
						sprintf(buf,
								"Far to the %s, you see $n leave %sward.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								direction2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
					}
			...
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir] == waf->a.shadow.edge) {
					...
					if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
						...
						sprintf(buf,
								"Very far to the %s, you see #5a %s#0 led by $n leave %sward.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								qe->group_str, direction2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
					} else {
						sprintf(buf,
								"Very far to the %s, you see $n leave %sward.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								direction2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
					}
				...
				if (tch->aiming_at == ch) {
					if (!(toward = is_moving_toward(ch, qe->dir, tch))
						...
						if ((IS_SET (tch->plr_flags, AUTO_FIRE) || (IS_NPC(tch)))
								...
								&& has_firearm(tch, 0)) {
							sprintf(buf,
									"Your quarry moves %sward, but before you lose sight. . .\n",
									direction2[qe->dir]);
							send_to_char(buf, tch);
	...
	if ((prevroom->sector_type == SECT_LEANTO
		...
		while (prevroom->contents) {
			...
			if (echo == 0)
				send_to_char(
						"You take your belongings with you as you leave.\n",
						ch);
			echo = 1;
		}
	}
	...
	if (ch->in_room && !ch->room) {
		send_to_gods(
				"Error: char moved room but didn't keep ch->room. Let Kith know.");
...
// Next dozen lines can be replaced by this 0208142142 -Nimrod
	...
	for (prog_obj = ch->room->contents; prog_obj;
			prog_obj = prog_obj->next_content) {
		if (prog_obj->ocues) {
	...
	if (IS_MORTAL (ch) && ch->speed != SPEED_IMMORTAL) {
		if ((prevroom->sector_type != SECT_LAKE
				...
				&& SWIM_ONLY(ch->room)) {
			send_to_char("\n", ch);
			act("You enter the water, and begin attempting to swim. . .", false,
					ch, 0, 0, TO_CHAR);
		...
		} else if ((prevroom->sector_type == SECT_LAKE
				|| prevroom->sector_type == SECT_RIVER
				|| prevroom->sector_type == SECT_OCEAN
				|| prevroom->sector_type == SECT_UNDERWATER)
				&& (ch->room->sector_type != SECT_LAKE
	...
	for (dir = 0; dir <= LAST_DIR; dir++) {
			...
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir] == waf->a.shadow.edge) {
					...
					if (IS_SET(qe->flags, MF_SNEAK)
						...
						qe->flags &= ~MF_SNEAK;
						sprintf(buf,
								"To the %s, you see $n arrive stealthily from %s.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								rev_d2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
					} else if (!IS_SET(qe->flags, MF_SNEAK)) {
						if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
							...
							sprintf(buf,
									"To the %s, you see #5a %s#0 led by $n arrives from %s.\n",
									direction[rev_dir[waf->a.shadow.edge]],
									qe->group_str, rev_d2[qe->dir]);
							act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
							target_sighted(tch, ch);
						}
						...
						// all heading in the same direction.
								...
								&& waf->a.shadow.edge == qe->dir
								&& GET_FLAG(tch, FLAG_LEAVING))) {
							sprintf(buf,
									"To the %s, you see $n arrive from %s.\n",
									direction[rev_dir[waf->a.shadow.edge]],
									rev_d2[qe->dir]);
							act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
							target_sighted(tch, ch);
		...
		if (!(exit) || !(next_room = vnum_to_room(exit->to_room))
				|| IS_SET(exit->exit_info,
						EX_CLOSED) || IS_SET (qe->flags, MF_SNEAK))
			continue;
			...
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir] == waf->a.shadow.edge) {
					...
					if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
						...
						sprintf(buf,
								"Far to the %s, you see #5a %s#0 led by $n arrives from %s.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								qe->group_str, rev_d2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
						target_sighted(tch, ch);
						...
						sprintf(buf,
								"Far to the %s, you see $n arrive from %s.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								rev_d2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
						target_sighted(tch, ch);
			...
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir] == waf->a.shadow.edge) {
					...
					if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
						...
						sprintf(buf,
								"Very far to the %s, you see #5a %s#0 led by $n arrives from %s.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								qe->group_str, rev_d2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
						target_sighted(tch, ch);
						...
						sprintf(buf,
								"Very far to the %s, you see $n arrive from %s.\n",
								direction[rev_dir[waf->a.shadow.edge]],
								rev_d2[qe->dir]);
						act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
						target_sighted(tch, ch);
	...
	if (qe->event_time <= 0) {
		if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
			...
			sprintf(buf, "$n arives from %s, #5a %s#0 following behind.",
					rev_d2[qe->dir], group_str);
			act(buf, true, ch, 0,
					drag_af ? (CHAR_DATA *) drag_af->a.spell.t : 0,
		...
		if (is_mounted(ch) && IS_RIDEE(ch)) {
			...
			if (get_second_affect(ch, SA_GFOLLOW, NULL)
				...
				;
					...
					|| get_second_affect(ch->mount, SA_LEAD, NULL)) {
				sprintf(buf, "$n %ss $N in from %s, #5a %s#0 following behind.",
						mount_speeds[
								ch->mount->pc ?
										ch->mount->pc->mount_speed : ch->speed],
						rev_d2[qe->dir], group_str);
			...
			if (!IS_SET(qe->flags, MF_SNEAK)) {
				char *move_names[] =
						{ "walks", "trudges", "paces", "jogs", "runs",
								"sprints", "blasts", "swims", "crawls", "floats", };

				if (lookup_race_int(ch->race,
						RACE_MOVEMENT) == RACE_MOVEMENT_FLY) {
					move_names[qe->speed_type] = "flies";
				} else if (lookup_race_int(ch->race,
						RACE_MOVEMENT) == RACE_MOVEMENT_SLITHER) {
					move_names[qe->speed_type] = "slithers";
				} else if (lookup_race_int(ch->race,
						RACE_MOVEMENT) == RACE_MOVEMENT_SCUTTLE) {
					move_names[qe->speed_type] = "scuttles";
				} else if (lookup_race_int(ch->race,
						RACE_MOVEMENT) == RACE_MOVEMENT_DRIFT) {
				...
				if (!IS_SET(ch->act, ACT_FLYING)) {
					if (get_second_affect(ch, SA_GFOLLOW, NULL)) {
						;
							...
							&& !ch->following) {
						sprintf(buf,
								"$n %s in from %s%s, #5a %s#0 following behind.",
								move_names[qe->speed_type], rev_d2[qe->dir],
	...
	if (!check_climb(ch)) {
		...
		while (IS_SET(ch->room->room_flags, CLIMB)
			...
			if (!room_exit
					|| IS_SET(room_exit->exit_info,
							EX_ISDOOR) || IS_SET (room_exit->exit_info, EX_ISGATE))
				break;
		...
		for (i = 1; i <= number(1, 5) * rooms_fallen; i++) {
			died += wound_to_char(ch, figure_location(ch, HITLOC_LOLEGS),
	...
	if (ch->room && ch->room->enviro_type && ch->room->enviro_power) {
		if (!number(0, 2))
			object__enviro(ch, NULL, ch->room->enviro_type,
					ch->room->enviro_power / 2, HITLOC_NONE);
	}
	...
	for (prog_obj = ch->room->contents; prog_obj;
		...
		if (!IS_SET(qe->flags,
				MF_TOEDGE) && (GET_ITEM_TYPE(prog_obj) == ITEM_TRAP) && IS_MORTAL(ch)) {
			// ... and the direction set equals the direction the person is entering from...
			if ((prog_obj->o.od.value[1] == rev_dir[qe->dir])
					&& ((prog_obj->o.od.value[0] >= 1)
							|| (prog_obj->o.od.value[0] <= -1))) {
...
void exit_room(CHAR_DATA * ch, int dir, int flags, int leave_time,
	...
	char buf[AVG_STRING_LENGTH];
	char *speed_type_names[] = { "walking", "wandering", "slowly walking",
			"jogging", "running", "sprinting", "swimming", "crawling",
			"floating", };
	...
	if (ch->speed == SPEED_IMMORTAL) {
		...
		if (is_mounted(ch) && IS_RIDEE(ch)) {
			...
			sprintf(buf, "You make $N %s %s.",
					mount_speeds[
							rider->pc ? rider->pc->mount_speed : mount->speed],
					dirs[dir]);
			...
			if (do_group_size(rider) > 5) {
				if (!rider->following) {
					sprintf(buf,
							"$n starts %s $N %s, #5a %s#0 following behind $m.",
							mount_speeds_ing[
									rider->pc ?
											rider->pc->mount_speed :
					...
					add_second_affect(SA_LEAD, leave_time, rider, NULL, NULL,
							0);
				} else {
					add_second_affect(SA_GFOLLOW, leave_time, rider, NULL, NULL,
							0);
					add_second_affect(SA_GFOLLOW, leave_time, mount, NULL, NULL,
							0);
				}
			} else {
				sprintf(buf, "$n starts %s $N %s.",
						mount_speeds_ing[
								rider->pc ?
										rider->pc->mount_speed : mount->speed],
						dirs[dir]);
	...
	for (prog_obj = ch->room->contents; prog_obj;
		...
		if (!IS_SET(qe->flags,
			...
			if ((prog_obj->o.od.value[1] == qe->dir)
					&& ((prog_obj->o.od.value[0] >= 1)
							|| (prog_obj->o.od.value[0] <= -1))) {
...
int calc_movement_charge(CHAR_DATA * ch, int dir, int wanted_time, int flags,
	...
	if (dir == UP && IS_SET(target_room->room_flags, FALL) &&
	!get_affect (ch, MAGIC_AFFECT_LEVITATE)
	&& !IS_SET (ch->act, ACT_FLYING))
		return 0;

	if (dir == UP && IS_SET(target_room->room_flags, CLIMB) &&
	IS_ENCUMBERED (ch) &&
	!get_affect (ch, MAGIC_AFFECT_LEVITATE)
	&& !IS_SET (ch->act, ACT_FLYING))
		return 0;
	...
	*walk_time += terrain_walk_time[ch->room->sector_type];
	...
	if ((get_equip(ch, WEAR_PRIM) && get_equip(ch, WEAR_SEC))
		...
		*walk_time += 3;
	} else if (get_equip(ch, WEAR_PRIM) || get_equip(ch, WEAR_SEC)
			|| get_equip(ch, WEAR_SHIELD)) {
		*walk_time += 2;
	}
...
void initiate_move(CHAR_DATA * ch) {
	...
	char *move_names[] = { "walking", "wandering", "slowly walking", "jogging",
			"running", "sprinting", "blasting", "swimming", "crawling",
			"floating" };
	char *move_names2[] = { "walk", "wander", "pace", "jog", "run", "sprint",
			"blast", "swim", "crawl", "float" };
	...
	if (IS_HITCHEE(ch)) {
		...
		if (!GET_FLAG(ch->hitcher, FLAG_LEAVING)) {
			...
			if (IS_RIDEE(ch))
				act("Your mount is hitched to $N.", true, ch->mount, 0,
						ch->hitcher, TO_CHAR);

			act("You can't move while hitched to $N.", true, ch, 0, ch->hitcher,
					TO_CHAR);
	...
	if (ch->fighting) {
		...
		if (((IS_SET(ch->act, ACT_AGGRESSIVE)
				...
				&& is_he_here(ch, ch->following,
						true) && GET_FLAG (ch->following, FLAG_FLEE)) {
	...
	if (!room_exit || !(target_room = vnum_to_room(room_exit->to_room))
			|| (IS_SET(room_exit->exit_info, EX_CLOSED)
					&& ch->room->secrets[dir] && IS_MORTAL(ch))) {
	...
	if (dir == UP
			&& !get_affect(ch,
					MAGIC_AFFECT_LEVITATE) && !IS_SET (ch->act, ACT_FLYING)) {
	...
	if (!(target_room->capacity == 0)) {
		if ((sa = get_second_affect(ch, SA_PUSH, NULL)) && sa->info2 == 1) {
			remove_second_affect(sa);
		} else if (!room_avail(target_room, ch) && !(GET_TRUST(ch))
	...
	if (nPartyRoom && ch->in_room == nPartyRoom
						...
						if (obj->in_obj->in_obj->carried_by == ch
							...
							bIsMaskHolder = 1;
								...
								&& (obj->in_obj->in_obj->in_obj->carried_by
										== ch
										|| obj->in_obj->in_obj->in_obj->equiped_by
												== ch)) {
							bIsMaskHolder = 1;
						}
					}
	...
	if (IS_MORTAL(
			ch) && (!IS_SET (flags, MF_SWIM) && SWIM_ONLY (vnum_to_room (room_exit->to_room))
	...
	if (IS_MORTAL(
			ch) && (!IS_SET (flags, MF_FLOAT) && FLOAT_ONLY (vnum_to_room (room_exit->to_room))
	...
	if (IS_SET(ch->act, ACT_MOUNT) && IS_RIDEE(ch)
			&& !ride_mount(ch, ch->mount, ch->room->sector_type,
					target_room->sector_type, dir)) {
		clear_moves(ch);
		return;
	}

	needed_movement = calc_movement_charge(ch, dir, wanted_time, flags, &speed,
			&speed_name, &walk_time);
	...
	if (IS_MORTAL(ch) && eq_penalty && ch->room->sector_type != SECT_INSIDE
		...
		for (tch = ch->room->people; tch; tch = tch->next_in_room) {
			if ((af = get_affect(tch, MAGIC_WATCH))) {
				if ((CHAR_DATA *) af->a.spell.t == ch) {
					sprintf(buf, "You spy $N beginning to sneak %s.",
							dirs[dir]);
	...
	if (((ch->room->sector_type == SECT_INSIDE
			...
			&& !IS_SET(flags, MF_SNEAK) && ch->speed != SPEED_IMMORTAL)
			|| (!IS_MORTAL(ch) && ch->speed != SPEED_IMMORTAL
					&& !IS_SET(flags, MF_SNEAK))) {
		if (do_group_size(ch) > 5) {
			if (!ch->following) {
				sprintf(buf1,
						"$n begins %s %s%s%s, #5a %s#0 following behind $m.",
						move_names[ch->speed], dirs[dir], suffix, travel_str,
						group_str);
				act(buf1, true, ch, 0, 0, TO_ROOM | _ACT_FORMAT);

				sprintf(buf,
						"You begin %s %s%s%s, #5a %s#0 following behind.\n",
						move_names[ch->speed], dirs[dir], suffix, travel_str,
						group_str);
				send_to_char(buf, ch);
	...
	if (ch->room->sector_type != SECT_INSIDE
			&& !IS_SET(ch->room->room_flags,
					INDOORS) && IS_MORTAL (ch) && !IS_SET (flags, MF_SNEAK) && !IS_SUBDUER (ch)
		...
		default:
			...
			if (do_group_size(ch) > 5) {
				if (is_group_leader(ch)) {
					sprintf(buf1,
							"$n begins %s %s%s%s, #5a %s#0 following behind $m.",
							move_names[ch->speed], dirs[dir], suffix,
							travel_str, group_str);
					act(buf1, true, ch, 0, 0, TO_ROOM | _ACT_FORMAT);
					sprintf(buf,
							"You begin %s %s%s%s, #5a %s#0 following behind.\n",
							move_names[ch->speed], dirs[dir], suffix,
					...
					sprintf(buf, "You begin %s %s%s%s.\n",
							...
							travel_str);
					send_to_char(buf, ch);
					add_second_affect(SA_GFOLLOW, exit_speed, ch, NULL, NULL,
							0);
				}
			} else {
				sprintf(buf1, "$n begins %s %s%s%s.", move_names[ch->speed],
	...
	exit_room(ch, dir, flags, exit_speed, enter_speed, speed_name,
			needed_movement, strlen(travel_str) ? str_dup(travel_str) : NULL,
...
int isguarded(ROOM_DATA * room, int dir) {
	...
	if (IS_SET(room->room_flags, BIG_ROOM) || IS_SET(room->room_flags, LAWFUL)) {
		thresh = 3;
	}
...
void do_move(CHAR_DATA * ch, char *argument, int dir) {
	...
	if ((af = get_affect(ch, AFFECT_GROUP_RETREAT))
		...
		if (*argument != '!' && ch->in_room == ch->following->in_room) {
			sprintf(buf,
					"If you wish to move away from and stop following $N, type #6%s !#0 to confirm it.",
					command);
			act(buf, false, ch, 0, ch->following, TO_CHAR | _ACT_FORMAT);
			return;
		}
		if (ch->following->in_room == ch->in_room)
			act("$N is no longer following you.", false, ch->following, 0, ch,
					TO_CHAR | _ACT_FORMAT);
		ch->following = 0;
		ch->formation = 0;
	}
	...
	if (IS_SET(ch->plr_flags, NEW_PLAYER_TAG) && ch->room->dir_option[dir]
			...
			&& IS_SET(
					vnum_to_room(ch->room->dir_option[dir]->to_room)->room_flags,
					FALL) && !IS_SET(ch->act, ACT_FLYING) && *argument != '!') {
		sprintf(buf,
	...
	if ((af = get_soma_affect(ch, SOMA_SHAKES))) {
		if ((number(0, 25) > GET_CON(ch) || number(0, 25) > GET_WIS(ch))
			...
			send_to_char(
					"You try to move your body, but you are wracked by a sudden spasm of pain and fall to the ground.\n",
					ch);
			sprintf(buf,
					"$n twitches violently, limbs spasming as $e collapses to the ground.");
			act(buf, true, ch, 0, 0, TO_ROOM);
...
void do_open(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (generic_find(type, FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &victim, &obj)) {
		...
		if (!IS_SET(EXIT (ch, door)->exit_info,
				EX_ISDOOR) && !IS_SET (EXIT (ch, door)->exit_info, EX_ISGATE))
			...
			if (EXIT (ch, door)->keyword && strlen(EXIT (ch, door)->keyword)) {
				if (!second_affect_active && ch->fighting) {
					sprintf(buf, "open %s %s", EXIT (ch, door)->keyword,
							dirs[door]);
					add_second_affect(SA_COMBAT_DOOR, number(5, 8), ch, NULL,
							buf, ch->in_room);
					sprintf(buf,
							"You attempt to move towards the %s $T in order to to open it.",
							relative_dirs[door]);
				...
				if (!second_affect_active && ch->fighting) {
					...
					add_second_affect(SA_COMBAT_DOOR, number(5, 8), ch, NULL,
							buf, ch->in_room);
					sprintf(buf,
							"You attempt to move towards the %s door in order to to open it.",
							relative_dirs[door]);
					act(buf, false, ch, 0, 0, TO_CHAR | _ACT_FORMAT);
					return;
				}
...
void do_close(CHAR_DATA * ch, char *argument, int cmd) {
	...
	else if (generic_find(type, FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &victim,
			&obj)) {
			...
			if ((other_room = EXIT (ch, door)->to_room) != NOWHERE)
				if ((back = vnum_to_room(other_room)->dir_option[rev_dir[door]]))
					if (back->to_room == ch->in_room) {
						back->exit_info |= EX_CLOSED;
						if (back->keyword && strlen(back->keyword)) {
							sprintf(buf, "The %s %s closes quietly.\n",
									relative_dirs[rev_dir[door]],
									back->keyword);
							send_to_room(buf, EXIT (ch, door)->to_room);
						} else {
							sprintf(buf, "The %s door closes quietly.\n",
									relative_dirs[rev_dir[door]]);
							send_to_room(buf, EXIT (ch, door)->to_room);
						}
					}
	...
	if (!*type)
		...
		if (!IS_SET(EXIT (ch, door)->exit_info,
			...
			if (EXIT (ch, door)->keyword && strlen(EXIT (ch, door)->keyword)) {
				...
				//dwellings don't have directions for doors
				if (ch->in_room < 100000)
					sprintf(buf, "You unlock the %s door.",
							relative_dirs[door]);
				else
...
void do_pick(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (locked_obj) {
		...
		if (GET_ITEM_TYPE (locked_obj) == ITEM_CONTAINER) {
			...
			if (!IS_SET(locked_obj->o.container.flags, CONT_LOCKED)) {
				act("As you start, you discover $p is already unlocked.", false,
						ch, locked_obj, 0, TO_CHAR);
				return;
			}
		} else {
		...
		act("You begin picking the lock of $p.", false, ch, locked_obj, 0,
				TO_CHAR);
	...
	if (!IS_SET(EXIT (ch, dir)->exit_info,
			EX_ISDOOR) && !IS_SET (EXIT (ch, dir)->exit_info, EX_ISGATE)) {
	...
	act("You try to pick the $T.", false, ch, 0, EXIT (ch, dir)->keyword,
			TO_CHAR);

	if (number(1, 100) > ch->skills[SKILL_PICK]) {

		act("$n uses $s tools on the $T.", true, ch, 0, EXIT (ch, dir)->keyword,
				TO_ROOM);
...
void delayed_pick_obj(CHAR_DATA * ch) {
	...
	if (GET_ITEM_TYPE (locked_obj) == ITEM_CONTAINER) {
		...
		if ((roll = number(1, 100))
				> ch->skills[SKILL_PICK]
						- locked_obj->o.container.pick_penalty) {
		...
		if ((roll = number(1, 100))
				> ch->skills[SKILL_PICK] - locked_obj->o.od.value[4]) {
			if (!(roll % 5) && (tobj = get_carried_item(ch, ITEM_LOCKPICK))) {
				act("You fail miserably, snapping your pick in the process!",
						false, ch, 0, 0, TO_CHAR);
				act("$n mumbles as $s lockpick snaps.", true, ch, 0, 0,
...
void delayed_pick(CHAR_DATA * ch) {
	...
	if ((roll = number(1, 100)) > ch->skills[SKILL_PICK] - EXIT (ch,
			dir)->pick_penalty) {
		if (!(roll % 5) && (tobj = get_carried_item(ch, ITEM_LOCKPICK))) {
			act("You fail miserably, snapping your pick in the process!", false,
					ch, 0, 0, TO_CHAR);
			act("$n mumbles as $s lockpick snaps.", true, ch, 0, 0,
			TO_ROOM | _ACT_FORMAT);
			extract_obj(tobj);
...
void do_enter(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (*buf && (obj = get_obj_in_list_vis(ch, buf, ch->room->contents))
		...
		argument = one_argument(argument, buf);
		if (IS_SET(obj->obj_flags.extra_flags,
				ITEM_PITCHABLE) && !IS_SET (obj->obj_flags.extra_flags, ITEM_PITCHED)) {
...
void do_stand(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (*buf) {
		if (!str_cmp(buf, "centre") || !str_cmp(buf, "clear")
				|| !str_cmp(buf, "c")) {
			if ((af = get_affect(ch, AFFECT_SHADOW))) {
	...
	if (get_soma_affect(ch, SOMA_SHAKES)
		...
		send_to_char(
				"Your body twitches and flails, your legs refusing to obey you!\n",
				ch);
	}
	...
	if ((static_cast<float>(ch->shock) / ch->max_shock) < 0.2000) {
		send_to_char(
				"You are too badly shaken and traumatised to stand right now.\n",
				ch);
		return;
	}
	...
	case SIT:
		if ((af = get_affect(ch, AFFECT_COVER))) {
			clear_pmote(ch);
			if (af->a.cover.obj
					&& is_obj_in_list(af->a.cover.obj, ch->room->contents)) {
				std::string temp;
				temp = "You rise up from behind "
						+ MAKE_STRING(obj_short_desc(af->a.cover.obj)) + ".";
				first_person.assign(temp);
				temp = "#0 rises up from behind "
						+ MAKE_STRING(obj_short_desc(af->a.cover.obj)) + ".";
			...
			if (evaluate_emote_string(ch, &first_person, third_person,
					argument))
				GET_POS (ch) = STAND;
			else
				return;
	...
	default:
		act("You stop floating around, and put your feet on the ground.", false,
				ch, 0, 0, TO_CHAR);
		act("$n stops floating around, and puts $s feet on the ground.", true,
				ch, 0, 0, TO_ROOM);
		break;
	}
...
void do_sit(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (obj) {
		...
		if (GET_POS (ch) != STAND) {
			act("You must be standing before you can sit at $p.", false, ch,
					obj, 0, TO_CHAR);
			return;
		}
	...
	case POSITION_FIGHTING:
		act("YOU'RE FIGHTING!  YOU'LL GET SLAUGHTERED!!!", false, ch, 0, 0,
				TO_CHAR);
...
void do_wake(CHAR_DATA * ch, char *argument, int not_noisy) {
	...
	if (*buf && tch != ch) {
		...
		if ((tch = being_dragged(ch))) {
			act("You awaken to discover $N was dragging you!", false, ch, 0,
					tch, TO_CHAR);
	...
	if (GET_POS (ch) > POSITION_SLEEPING)
		...
		if ((tch = being_dragged(ch))) {
			if (GET_POS (ch) == REST) {
				act("You awake to discover $N is dragging you!", false, ch, 0,
						tch, TO_CHAR);
			} else {
				act("You awaken to discover $N was dragging you!", false, ch, 0,
						tch, TO_CHAR);
...
void do_sail(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (is_room_affected(
			vnum_to_room(EXIT (ch->vehicle, dir)->to_room)->affects,
			MAGIC_ROOM_FLOOD)) {
		...
		if (needed < 30) {
			...
			if (track->from_dir != track->to_dir)
				sprintf(buf + strlen(buf),
						...
						specific_name(track->race),
						IS_SET (track->flags,
								BLOODY_TRACK) ?
								", #1pooled with blood#0, " : "",
						track_age(track->hours_passed), speed_adj(track->speed),
...
void do_track(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (IS_SET(ch->room->dir_option[dir]->exit_info, EX_LOCKED)) {
		if (ch->room->dir_option[dir]->key) {
			for (tch = ch->room->people; tch; tch = tch->next_in_room) {
				if (get_obj_in_list_num(ch->room->dir_option[dir]->key,
						tch->right_hand)
...
void do_drag(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (victim && AWAKE(victim)) {
		act("$N is conscious, you can't drag $M.", false, ch, 0, victim,
				TO_CHAR);
		return;
	}
	...
	if (obj && !IS_SET(obj->obj_flags.wear_flags, ITEM_TAKE)) {
		act("$p is firmly attached.  You can't drag it.", false, ch, obj, 0,
				TO_CHAR);
		return;
	}
...
int can_mount(CHAR_DATA * ch, CHAR_DATA * mount) {
	if (CAN_SEE(ch, mount) && IS_SET(mount->act, ACT_MOUNT)
...
void do_hitch(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!*buf && IS_HITCHER(ch)) {
		act("You currently have $N hitched.", true, ch, 0, ch->hitchee,
				TO_CHAR);
		return;
	}
	...
	if (IS_HITCHEE(hitch)) {
		act("$N is already hitched to $3.", true, ch,
				(OBJ_DATA *) hitch->hitcher, hitch, TO_CHAR);
		return;
	}
	...
	if (!is_vehicle) {
		if (!(bridle = get_equip(hitch, WEAR_NECK_1))) {
			act("$N doesn't have a bridle to grab.", true, ch, 0, hitch,
					TO_CHAR);
			return;
		}

		if (GET_ITEM_TYPE (bridle) != ITEM_BRIDLE) {
			act("$N is wearing $o, which is not a bridle.", true, ch, bridle,
					hitch, TO_CHAR);
			return;
		}
	}
	...
	if (IS_HITCHER(hitcher)) {
		act("$N has an animal hitched already.", true, ch, 0, hitcher, TO_CHAR);
		return;
	}

	hitch_char(hitcher, hitch);

	act("$n hitches $N to $3.", false, ch, (OBJ_DATA *) hitcher, hitch,
			TO_NOTVICT);
	act("$N hitches $3 to you.", false, ch, (OBJ_DATA *) hitch, hitcher,
			TO_VICT);
	act("You hitch $3 to $N.", true, ch, (OBJ_DATA *) hitch, hitcher, TO_CHAR);
	act("$N hitches you to $3.", true, hitch, (OBJ_DATA *) hitcher, ch,
			TO_CHAR);
}
...
void do_unhitch(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (hitch->hitcher != ch && !IS_SET(hitch->hitcher->act, ACT_MOUNT)) {
		act("$N is holding that animal's reigns.", true, ch, 0, hitch->hitcher,
				TO_CHAR);
		return;
	}
...
void do_mount(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (IS_SET(mount->affected_by, AFF_TRANSPORTING)) {
		act("There's no room on $N's back for you!.", false, ch, 0, mount,
				TO_CHAR);
		return;
	}
	...
	if (!real_skill(ch, SKILL_HANDLE) && mount->skills[SKILL_HANDLE] < 33) {
		act("$N is too wild for you to even try.", false, ch, 0, mount,
				TO_CHAR);
		return;
	}

	if (real_skill(mount, SKILL_HANDLE) < 33
			&& !skill_use(ch, SKILL_HANDLE, 0)) {
		act("$N backs away just as you attempt to mount it.", false, ch, 0,
				mount, TO_CHAR);
		act("You back away as $N approaches you.", false, mount, 0, ch,
				TO_CHAR);
		act("$N backs away when $n attempts to mount it.", false, ch, 0, mount,
				TO_NOTVICT);
		return;
	}

	if (mount->skills[SKILL_HANDLE] < 90
			&& !skill_use(mount, SKILL_HANDLE, -75)) {
		act("You manage to straddle $N, but it quickly throws you off.", false,
				ch, 0, mount, TO_CHAR);
		act("$n is thrown to the ground after attempting to mount $N.", false,
				ch, 0, mount, TO_NOTVICT);
		act("You throw $N to the ground after $E attempts to mount you.", false,
				mount, 0, ch, TO_CHAR);
		return;
	}
...
void do_bridle(CHAR_DATA * ch, char *argument, int cmd) {
	...
	if (!IS_SET(mount->act, ACT_MOUNT)) {
		act("$N is not a mount; it can't be bridled.", false, ch, 0, mount,
				TO_CHAR);
		return;
	}
	...
	if ((obj = get_equip(mount, WEAR_NECK_1))) {
		act("$N already has $o around its neck.", false, ch, obj, mount,
				TO_CHAR);
		return;
	}
	...
	if (!real_skill(mount, SKILL_HANDLE)) {
		if (ch->skills[SKILL_HANDLE] < 30) {
			act("$N shies away as attempt to slip $o onto $M.", false, ch,
					bridle, mount, TO_CHAR);
			return;
		}
		if (!skill_use(ch, SKILL_HANDLE, -30)) {
			act("$N kicks you as you attempt to bridle $M!", false, ch, 0,
					mount, TO_CHAR);
			act("You kick $N when $E tries to bridle you.", false, mount, 0, ch,
					TO_CHAR);
			act("$N kicks $n as $E attempts to bridle $M.", false, ch, 0, mount,
					TO_NOTVICT);
			wound_to_char(ch, figure_location(ch, HITLOC_BODY), dice(2, 12), 3,
					0, 0, 0);
			return;
		}
	}
...
void dump_rider(CHAR_DATA * rider, int forced) {
	...
	if (!skill_use(rider, SKILL_HANDLE, 0)) {
		if (forced == 2) {
			act(
					"You buck and thrash as the missile strikes, throwing $N off your back!",
					false, rider->mount, 0, rider, TO_CHAR);
			act(
					"$N bucks and thrashes as the missile strikes, throwing you off!",
					false, rider, 0, rider->mount, TO_CHAR);
			act(
					"$N bucks and thrashes as the missile strikes, throwing $n off its back.",
					true, rider, 0, rider->mount, TO_NOTVICT);
		} else {
			act("You throw $N off your back!", false, rider->mount, 0, rider,
					TO_CHAR);
			act("$N bucks and throws you off!", false, rider, 0, rider->mount,
					TO_CHAR);
			act("$N bucks $n off its back.", true, rider, 0, rider->mount,
					TO_NOTVICT);
		}
		...
		if (wound_to_char(rider, figure_location(rider, HITLOC_LOLEGS), 1, 3, 0,
				0, 0))
			return;
	...
	act("You buck as you pretend to be REAL scared.", false, rider->mount, 0,
			rider, TO_CHAR);
	act("$N bucks and tries to throw you off!", false, rider, 0, rider->mount,
			TO_CHAR);
	act("$N bucks and tries to throw $n off its back.", true, rider, 0,
			rider->mount, TO_NOTVICT);
}
...
void watched_action(CHAR_DATA *ch, char *arg, int dist, int mode) {
	...
	for (dir = 0; dir <= LAST_DIR; dir++) {
		...
		for (tch = next_room->people; tch; tch = tch->next_in_room) {
			if ((waf = get_affect(tch, AFFECT_WATCH_DIR))) {
				if (rev_dir[dir]
						== waf->a.shadow.edge/* && could_see(tch, ch)*/) {
					target_sighted(tch, ch);
					if (!mode) {
						sprintf(buf, "To the %s, you see $n %s",
							...
							*heard = '\0';
							...
							for (size_t y = 0; y <= strlen(arg); y++) {
								if (arg[y] == '\"') //if this is true, we have found the first quotation mark
									...
									{
										sprintf(speech + strlen(speech), "%c",
												arg[j]);
										j++;
									}
									...
									sprintf(heard + strlen(heard), "%s",
											speech);
									y = j;
								} else {
									sprintf(heard + strlen(heard), "%c",
											arg[y]);
								}
							}
							sprintf(buf, "To the %s, %s",
									direction[rev_dir[waf->a.shadow.edge]],
									heard);
						} else {
							sprintf(buf, "To the %s, %s",
...
void do_push(CHAR_DATA *ch, char *argument, int cmd) {
	...
	if (get_second_affect(ch, SA_PUSH, NULL)) {
		send_to_char(
				"You need a few more moments breath before you can attempt to push again.\n",
				ch);
		return;
	}
	...
	if (dest && (capac = dest->capacity) <= 0 && room_avail(dest, ch)) {
		send_to_char(
				"The room has plenty of room for you to enter without needing to push.\n",
				ch);
		return;
	}
	...
	if (IS_SET(ch->act, ACT_SQUEEZER)) {
		...
		send_to_char("You try to force and squeeze your way in, but fail.\n",
				ch);
		sprintf(buf,
				"$n attempts to force and squeeze $s way %sward, but fails.",
				direction[dir]);
		act(buf, false, ch, 0, 0, TO_ROOM | _ACT_FORMAT);
