...
run_the_game (int port)
{
    ...
    if (!(fp = fopen ("booting", "r")))
    {
        ...
        fprintf( stdout, "Crash loop check initiated.\n" );
    }
    else
    {
    ...
    if (engine.in_play_mode ())
    {
        ...
        {
            fprintf (fp,
                     "Our game server seems to be down. Our apologies for any inconvenience.\n");
            fclose (fp);
        }
    }
	...
	//		add_a_minute();
        ...
        if (!((pulse + 9) % (PULSE_AUTOSAVE * 5)))
        {
            if (!engine.in_build_mode ())
            {
                ...
                {
                    if (!number (0, 19) && is_grunge_arena_clear ())
                    {
                        //morgul_arena_wargs ();
                    }
                }
        ...
        if (!(pulse % (SECOND_PULSE * 60 * 30))) //every 2 IG hours
        {
            if (!grunge_arena_fight
                    && engine.in_play_mode ()
                    && is_grunge_arena_clear())
            {
                grunge_arena_first ();
                grunge_arena_time = (int) time (0);
                grunge_arena_matches++;
            }
        }
...
vote_notifications (void)
{
    ...
    if ((result = mysql_store_result (database)) != NULL)
    {
        ...
        while ((row = mysql_fetch_row (result)))
        {
            for (d = descriptor_list; d; d = d->next)
            {
                if (!d->strClientIpAddr || !d->acct || !d->character
...
update_website (void)
{
    ...
    for (d = descriptor_list; d; d = d_next)
    {
        ...
        if (IS_MORTAL (d->character))
            count++;
        mysql_safe_query
        ("INSERT INTO players_online VALUES ('%s', '%s', '%s', %d, %d)",
         d->acct->name.c_str (), d->character->tname,
    ...
    if (engine.in_play_mode ())
    {
        ...
        if (atoi (row[0]) < count)
        {
            mysql_safe_query ("UPDATE newsletter_stats SET most_online = %d",
                              count);
        }
        ...
        mysql_safe_query ("UPDATE server_statistics SET max_players = %d",
                          count_max_online);

        mysql_safe_query ("UPDATE server_statistics SET morgul_arena = %d",
                          grunge_arena_time);
    }
}

void
send_to_room_excluding (const char *message, const CHAR_DATA *ch)
...
void
send_to_char (const char *message, CHAR_DATA * ch)
{
    DESCRIPTOR_DATA *d;
...
send_to_gods (const char *message)
{
    ...
    if (!str_cmp(message, "(null)"))
	{
		system_log ("Null message echoed", true);
	}
...
send_to_guardians (char *message, unsigned short int flag)
{
    ...
    for (d = descriptor_list; d; d = d->next)
    {
        ...
        if (!d->connected && !IS_MORTAL (d->character)
...
send_to_room (char *message, int room_num)
{
    ...
    if (!(room = vnum_to_room (room_num)))
        return;

    reformat_string (message, &formatted);

    for (tch = room->people; tch; tch = tch->next_in_room)
        if (tch->descr() && !IS_SET (tch->act, PLR_QUIET))
            write_to_q (formatted, &tch->descr()->output);

    mem_free (formatted);
}

void
send_to_room_unformatted (char *message, int room_num)
{
    CHAR_DATA *tch;
    ROOM_DATA *room;
    ...
    for (tch = room->people; tch; tch = tch->next_in_room)
        if (tch->descr() && !IS_SET (tch->act, PLR_QUIET))
            write_to_q (message, &tch->descr()->output);
}
     ...
     OBJ_DATA * obj, void *vict_obj, int type)
{
    ...
    for (; to; to = to->next_in_room)
    {
        ...
        if (ch != to && !IS_MORTAL (ch) && IS_MORTAL (to)
                && GET_FLAG (ch, FLAG_WIZINVIS) && !IS_NPC(to))
        {
            continue;
        }
        ...
        if (to->descr() &&
                ...
                (are_grouped (to, ch) || !IS_SET (type, TO_GROUP)) &&
                ...
                !(IS_SET (type, TO_NOTVICT) && to == (CHAR_DATA *) vict_obj))
        {
            ...
            *(++point) = '\0';
            ...
            if (IS_SET (type, _ACT_COMBAT)
                    && IS_SET (to->plr_flags, COMBAT_FILTER)
                    && to->fighting != vict_obj && to->fighting != ch)
            {
            ...
            /// <li>  We do not send the buffer:
            /// <ul>
            /// <li>  \e IF this is a firefight message </li>
            /// <li>  \e AND the recipient filters firefight messages </li>
            /// <li>  \e AND the recipient is not fighting  the victim</li>
            /// <li>  \e AND the recipient is not fighting the actor</li>
            /// <li>  \e AND the victim is not fighting the recipient</li>
            /// <li>  \e AND the actor is not fighting the recipient</li>
            ...
            if (IS_SET (type, _ACT_FIREFIGHT) && IS_SET (to->plr_flags, FIREFIGHT_FILTER) && ((to->fighting && to->fighting != vict_obj) || !to->fighting) && to->fighting != ch)
            {
                if (vict_obj && ((CHAR_DATA *) vict_obj)->fighting == to)
                    ;
                else if (ch->fighting == to)
            ...
            if (IS_SET (type, _ACT_BLEED)
                    && IS_SET (to->plr_flags, COMBAT_FILTER)
                    && to->fighting && to->fighting != vict_obj && to->fighting != ch)
            {
                if (vict_obj && ((CHAR_DATA *) vict_obj)->fighting == to)
                    ;
                else if (ch->fighting == to)
            ...
            if (IS_SET (type, _HIGHLIGHT)
                    && IS_SET (to->plr_flags, HIGHLIGHT))
            {
                sprintf(buf, "%s", highlight(buf));
            }
...
do_gstat (CHAR_DATA * ch, char *argument, int cmd)
{
    ...
    *(tmstr + strlen (tmstr) - 1) = '\0';
    ...
    send_to_char (buf, ch);
    std::string svn_url = "$HeadURL: http://atonementrpi.com/var/www/svn/engine/trunk/comm.cpp $";
    svn_url.erase (0,33);
    svn_url.erase (svn_url.rfind ("/comm.cpp"));
...
prepare_copyover (int cmd)
{
    ...
    // Going to find out what's causing this rebooting mayhem.
    //if (engine.in_play_mode())
    //    abort();
    ...
    for (d = descriptor_list; d; d = d_next)
    {
        ...
        if (d->character && d->connected == CON_PLYNG && d->character->room
                && d->character->in_room != NOWHERE)
        {
            if (GET_POS (d->character) == POSITION_FIGHTING)
                ...
                fprintf (fp, "%d %s %s %s %d", d->hSocketFD, d->strClientHostname,
                         ...
                         GET_POS (d->character));
            count = 0;
            if (GET_POS (d->character) == POSITION_SITTING
...
copyover_recovery (void)
{
    ...
    for (;;)
    {
        ...
        if (!IS_SET (d->character->flags, FLAG_GUEST) && d->acct && d->acct->sound)
            d->character->sound++;

        if (d->character->pc && d->acct && d->acct->guide)
