...
set_fighting(CHAR_DATA* ch, CHAR_DATA* vict)
{
	...
	if (get_second_affect(vict, SA_INVOLCOVER, 0))
	{
		send_to_char("You try to rise to your feet...\n", vict);
		do_stand(vict, "", 0);
		remove_second_affect(get_second_affect(vict, SA_INVOLCOVER, 0));
	}
...
make_corpse(CHAR_DATA* ch)
{
	...
	if (lookup_race_int(ch->race, RACE_CORPSE))
	{
		...
		else if (ch->mob && ch->mob->carcass_vnum && (o = load_object(ch->mob->carcass_vnum)) != 0)
		{
...
do_aimstrike(CHAR_DATA* ch, char* argument, int cmd)
{
	...
	else
	{
		...
		add_second_affect(SA_AIMSTRIKE, delay, ch, 0, 0, bodypart);
...
do_combat_feint(CHAR_DATA* ch, char* argument, int cmd)
{
	...
	if (get_second_affect(ch, SA_FEINT, 0))
	{
	...
	add_second_affect(SA_FEINT, 60, ch, 0, 0, 0);
}
...
do_combat_ward(CHAR_DATA* ch, char* argument, int cmd)
{
	...
	else
	{
		...
		{
			act("Using $p, you force those attacking you to keep their distance.", true, ch, weapon, 0, TO_CHAR | _ACT_FORMAT | _ACT_COMBAT);
			act("$n uses $p to keep $s attackers at distance", true, ch, weapon, 0, TO_ROOM | _ACT_FORMAT | _ACT_COMBAT);
			...
			while (attackers > 0)
			{
				for (rch = ch->room->people; rch; rch = rch->next_in_room)
				{
					if (skill_use(ch, SKILL_SOLE_WIELD, 0) && rch->fighting == ch && ch->fighting != rch)
					{
						...
						stop_fighting(rch);
						add_second_affect(SA_WARDED, 60, rch, (OBJ_DATA *) ch, 0, 0);
					}
				}
				attackers--;
			}
...
do_combat_bash(CHAR_DATA* ch, char* argument, int cmd)
{
	...
	if (result < -40)
	{
		...
		{
			object__enviro(ch, 0, COND_DIRT, 3, HITLOC_NONE);
			object__enviro(ch, 0, COND_DUST, 6, HITLOC_NONE);
		}
	}
	else if (result < 0)
	{
	...
	else
	{
		...
		{
			object__enviro(victim, 0, COND_DIRT, 3, HITLOC_NONE);
			object__enviro(victim, 0, COND_DUST, 6, HITLOC_NONE);
		}
	}
...
hit_char(CHAR_DATA* ch, CHAR_DATA* victim, int strike_parm)
{
	...
	if (AWAKE (victim) && (get_affect(ch, MAGIC_HIDDEN)))
	{
		...
		if (sneak && hide)
		{
			...
			if (!(get_second_affect(ch, SA_AMBUSH, 0)))
				add_second_affect(SA_AMBUSH, 20, ch, 0, 0, 175);
		}
		else if (sneak || hide)
		{
			...
			if (!(get_second_affect(ch, SA_AMBUSH, 0)))
				add_second_affect(SA_AMBUSH, 20, ch, 0, 0, 125);
		}
		else
		{
			...
			if (!(get_second_affect(ch, SA_AMBUSH, 0)))
				add_second_affect(SA_AMBUSH, 1, ch, 0, 0, 50);
		}
	}
				...
				!get_equip(ch, WEAR_BOTH) && !get_equip(ch, WEAR_SEC))))
	{
		...
		else if (get_second_affect(ch, SA_FEINT, 0))
		{
			feint = true;
		}
		else
	...
	if (!killed && !ch->secondary_delay && get_equip(ch, WEAR_SEC))
	{
		...
		if (get_second_affect(ch, SA_FEINT, 0) && !feint)
			return;
		else if (get_second_affect(ch, SA_FEINT, 0) && feint)
		{
...
strike(CHAR_DATA* src, CHAR_DATA* tar, int attack_num, int mode)
{
	...
	OBJ_DATA* attack_weapon = 0;
	OBJ_DATA* defense_weapon = 0;
	OBJ_DATA* shield = 0;
	OBJ_DATA* attack_shield = 0;
	CHAR_DATA* mount;
	AFFECTED_TYPE* af = 0;
	SECOND_AFFECT* saf = 0;
	int bonus;
    ...
    */
	...
	saf = get_second_affect(src, SA_AMBUSH, 0);
	...
	if (get_second_affect(src, SA_FEINT, 0))
    ...
    */
	...
	if (get_second_affect(src, SA_FEINT, 0) && attack_num == 1)
		remove_second_affect(get_second_affect(src, SA_FEINT, 0));
               ...
               int off_style, int def_style, int org_loc, int att_diff, int def_diff)
{
	...
	int body_type = 0;
	//POISON_DATA *poison;
	AFFECTED_TYPE* invulnerability = 0;
	COMBAT_MSG_DATA* best_cm = 0;
	COMBAT_MSG_DATA* tcm = 0;
	CHAR_DATA* tch;
	OBJ_DATA* tobj;
	OBJ_DATA* eq;
	...
	*log_message = '\0';
	...
	else if (off_result == RESULT_STUMBLE)
	{
		...
		{
			object__enviro(src, 0, COND_DIRT, 3, HITLOC_NONE);
			object__enviro(src, 0, COND_DUST, 6, HITLOC_NONE);
		}
	...
	else if (off_result == RESULT_FUMBLE)
	{
		...
		if (!IS_SET (src->flags, FLAG_COMPETE))
		{
			if (attack_weapon == src->right_hand)
				...
				src->left_hand = 0;
			add_second_affect(SA_GET_OBJ, (10 - (GET_DEX(src) / 5) + number(2, 4)), src, attack_weapon, 0, 0);
			add_second_affect(SA_WEAR_OBJ, (13 - (GET_DEX(src) / 5) + number(2, 5)), src, attack_weapon, 0, 0);
	...
	if (def_result == RESULT_FUMBLE)
	{
		...
		if (!IS_SET (tar->flags, FLAG_COMPETE))
		{
			if (defense_weapon == tar->right_hand)
				...
				tar->left_hand = 0;
			add_second_affect(SA_GET_OBJ, (10 - (GET_DEX(tar) / 5) + number(2, 4)), tar, defense_weapon, 0, 0);
			add_second_affect(SA_WEAR_OBJ, (13 - (GET_DEX(tar) / 5) + number(2, 5)), tar, defense_weapon, 0, 0);
	...
	else if (def_result == RESULT_STUMBLE)
	{
		if (GET_POS (tar) == FIGHT || GET_POS (tar) == STAND)
		{
			...
			{
				object__enviro(tar, 0, COND_DIRT, 3, HITLOC_NONE);
				object__enviro(tar, 0, COND_DUST, 6, HITLOC_NONE);
			}
		}
		else
			def_result = RESULT_NONE;
    ...
    */
	...
	else
	{
		...
		if (!tar->fighting && !IS_SET (tar->act, ACT_VEHICLE))
		{
			...
			if (!AWAKE (tar) && GET_POS (tar) != POSITION_UNCONSCIOUS)
				do_wake(tar, "", 0);
			add_second_affect(SA_STAND, 8, tar, 0, 0, 0);
	...
	if (get_second_affect(src, SA_AIMSTRIKE, 0))
	{
		attack_delay = attack_delay * ((get_second_affect(src, SA_AIMSTRIKE, 0)->seconds) / 100) ;
		attack_delay += number(5, 8);
		remove_second_affect(get_second_affect(src, SA_AIMSTRIKE, 0));
	}
              ...
              int off_result, int* damage, int* location, int* original_damage)
{
	...
	*location = -1;
	...
	if (get_second_affect(src, SA_AIMSTRIKE, 0))
	{
		*location = get_second_affect(src, SA_AIMSTRIKE, 0)->info2;
	}
	else
	{
		i = number(1, 100);
	...
	if (get_second_affect(src, SA_AMBUSH, 0))
	{
		dam += 1;
	}
	...
	if (attack_weapon)
	{
		...
		object__enviro(src, 0, COND_BLOOD, dam / 4, HITLOC_HANDS);
		object__enviro(src, 0, COND_BLOOD, dam / 4, HITLOC_LOARMS);
	}
	else
	{
...
perform_violence(void)
{
	...
	if (combat_list)
	{
		...
		new_combat_list->next_fighting = 0;
	}
...
do_escape(CHAR_DATA* ch, char* argument, int cmd)
{
	...
	if (get_second_affect(ch, SA_ESCAPE, 0))
	{
	...
	add_second_affect(SA_ESCAPE, 10, ch, 0, 0, 0);
}
...
compete(CHAR_DATA* ch, CHAR_DATA* src, CHAR_DATA* tar, int iterations)
{
	...
	OBJ_DATA* src_both = 0;
	OBJ_DATA* tar_both = 0;
	OBJ_DATA* src_shield = 0;
	OBJ_DATA* tar_shield = 0;
	OBJ_DATA* obj;
	char buf[MAX_STRING_LENGTH];
...
sa_rescue(SECOND_AFFECT* sa)
{
	...
	else if (result == 0)
	{
		...
		add_second_affect(SA_RESCUE, 3, sa->ch, sa->obj, 0, 0);

		return;
	}

	else if (result == 1) /* Couldn't try...try asap */
		add_second_affect(SA_RESCUE, 1, sa->ch, sa->obj, 0, 0);
...
do_rescue(CHAR_DATA* ch, char* argument, int cmd)
{
	...
	sa = get_second_affect(ch, SA_RESCUE, 0);
	...
	if (result == 0)
	{
		...
		add_second_affect(SA_RESCUE, 3, ch, (OBJ_DATA *) friendPtr, 0, 0);
	}
	...
	else if (result == 1)
	{
		...
		add_second_affect(SA_RESCUE, 1, ch, (OBJ_DATA *) friendPtr, 0, 0);
	}
...
{
	OBJ_DATA* prim_eq = 0; // the names of prim_eq and sec_eq are confusing, as they are not directly 
	OBJ_DATA* sec_eq = 0; // related to primary and secondary armor.
	OBJ_DATA* cloth_eq = 0;
	OBJ_DATA* armor1 = 0;
	OBJ_DATA* armor2 = 0;
	CHAR_DATA* dch;
...
{
	OBJ_DATA* prim_armor = 0;
	OBJ_DATA* sec_armor = 0;

	OBJ_DATA* prim_worn = 0;
	OBJ_DATA* sec_worn = 0;
