...
extern const char* rev_d[] = { "the south,", "the west,", "the north,",
		"the east,", "below,", "above", "inside", "outside", "the southwest",
		"the southeast", "the northwest", "the northeast", "the downsouth",
		"the downwest", "the downnorth", "the downeast", "the downsouthwest",
		"the downsoutheast", "the downnorthwest", "the downnortheast",
		"the upsouth", "the upwest", "the upnorth", "the upeast",
		"the upsouthwest", "the upsoutheast", "the upnorthwest",
		"the upnortheast", "report to nimrod" };
...
int static_distance_mod(OBJ_DATA* sender, OBJ_DATA* receiver,
		bool radio_station) {
	if (!sender || !receiver)
		return 0;
	...
	if (sent_zone == 75 || sent_zone == 56 || sent_zone == 66 || sent_zone == 67
			|| sent_zone == 68) {
		grunge_bonus += 15;
	}

	if (rec_zone == 75 || rec_zone == 56 || rec_zone == 66 || rec_zone == 67
			|| rec_zone == 68) {
		grunge_bonus += 15;
	}
...
int static_it(OBJ_DATA* obj, char* source, char* target, bool radio_station,
		int dist_penalty) {
	int missed = 0;
	double count = 0;
	double failed = 0;
...
void heard_action(CHAR_DATA* ch, char* flavour, char* text, int dist,
		int mode) {
	int dir;
	CHAR_DATA* tch;
	AFFECTED_TYPE* waf;
				...
				if (rev_dir[dir]
					...
					if (!mode) {
						sprintf(buf, "To the %s, you overhear %s %s.\n",
								direction[rev_dir[waf->a.shadow.edge]], buf3,
								buf2);
					} else {
						sprintf(buf, "%s, %s",
								direction[rev_dir[waf->a.shadow.edge]], buf2);
					}
...
void do_eavesdrop(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (*buf) {
		...
		if (!(taf = get_affect(ch, AFFECT_SHADOW))
				|| (taf->a.shadow.edge != dir)) /* On the edge */
				{
			send_to_char(
					"You need to be standing by an edge to eavesdrop in that direction.\n",
					ch);
			return;
		}

		if (!(taf = get_affect(ch, AFFECT_WATCH_DIR))
				|| (taf->a.shadow.edge != dir)) /* On the edge */
				{
			send_to_char(
					"You need to be watching that direction before you can eavesdrop in that direction.\n",
					ch);
			return;
		}
	...
	if (!*buf) {
		if (!get_affect(ch, AFFECT_LISTEN_DIR)) {
			send_to_char(
					"You are not currently eavesdropping in any direction.\n",
					ch);
			return;
		} else {
...
void do_omote(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (argument[strlen(argument) - 1] == '!') {
		...
		for (size_t y = 0; y < strlen(argument) - 2; y++) {
			sprintf(original_argument + strlen(original_argument), "%c",
					argument[y]);
		}
	...
	if (get_obj_affect(obj, MAGIC_OMOTED)) {
		send_to_char(
				"That item has an omote-lock upon it - you will need to pick it up and drop it to place another omote.\n",
				ch);
		return;
	}
...
void do_think(CHAR_DATA* ch, char* argument, int cmd) {
	...
	*buf2 = '\0';
	...
	if (!*feel) {
		...
		sprintf(buf1, "#6%s#0 thinks, %s, \"%s\"", GET_NAME(ch), feel,
				argument);
		*buf = toupper(*buf);
		*buf1 = toupper(*buf1);
...
bool seen_hidde_emote(CHAR_DATA* src, CHAR_DATA* tar, int personal) {
	int roll = 0;
	int test = 0;
	AFFECTED_TYPE* af = NULL;
	...
	if ((af = get_affect(src, MAGIC_SIT_TABLE))
			&& is_at_table(tar, af->a.table.obj)) {
		test += 25;
	}

	// If you're watching them, you get +50

	if ((af = get_affect(tar, MAGIC_WATCH))
			&& (CHAR_DATA *) af->a.spell.t == src) {
		test += 50;
	}
...
bool personalize_hidden_string(CHAR_DATA* src, CHAR_DATA* tar, char* emote) {
	AFFECTED_TYPE* af;
	...
	*output = '\0';
	...
	if (target) {
		...
		if (seen_hidde_emote(src, target, 25)) {
			//send_to_gods("test1");
			if (!are_grouped(src, target) && !GET_TRUST(target) && (af =
					(get_affect(src, MAGIC_HIDDEN)))) {
				seen = true;
			}
...
bool personalize_hemote(CHAR_DATA* src, char* emote) {
	AFFECTED_TYPE* af;
	char desc[MAX_STRING_LENGTH] = { '\0' };
	...
	for (tch = src->room->people; tch; tch = tch->next_in_room) {
		...
		if (seen_hidde_emote(src, tch, 0)) {
			if (!are_grouped(src, tch) && !GET_TRUST(tch)
					&& (af = (get_affect(src, MAGIC_HIDDEN)))) {
				seen = true;
			}
...
void do_hemote(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (quotes) {
		if (IS_NPC(ch)
				&& !str_cmp(lookup_race_variable(ch->race, RACE_NAME),
						"robot")) {
		...
		if (seen) {
			if (get_affect(ch, MAGIC_HIDDEN)) {
				...
				send_to_char(
						...
						ch);
				act("$n reveals $mself.", false, ch, 0, 0,
				TO_ROOM | _ACT_FORMAT);
			}
		}
...
void do_emote(CHAR_DATA* ch, char* argument, int cmd) {
		...
		if (ch->in_room == StageRoom) {
			...
			*buf = tolower(*buf);
			for (int index = 0; index < 4; index = index + 1) {
				for (tch = vnum_to_room(AudienceRoom[index])->people; tch; tch =
						tch->next_in_room) {
	...
	if (quotes) {
		for (bugObj = ch->room->contents; bugObj; bugObj =
				bugObj->next_content) {
			if (GET_ITEM_TYPE(bugObj) == ITEM_E_BUG && bugObj->o.elecs.status) {
				...
				for (tobj = object_list; tobj; tobj = tobj->next) {
					...
					if (tobj->o.radio.channel != bugObj->o.radio.channel
							&& tobj->o.radio.channel / 10
									!= bugObj->o.radio.channel)
						continue;
					...
					for (robj = room->contents; robj; robj =
							robj->next_content) {
						if (robj->o.radio.status != 0 && tobj != robj
								&& robj != bugObj
								&& robj->o.radio.channel
										== tobj->o.radio.channel
								&& robj->o.radio.encrypt
										== tobj->o.radio.encrypt) {
					...
					if (!heard && !duplicated) {
						for (xch = room->people; xch; xch = xch->next_in_room) {
							if (xch->right_hand
									&& GET_ITEM_TYPE(xch->right_hand)
											== ITEM_E_RADIO) {
								if (xch->right_hand->o.radio.status != 0
										&& tobj != xch->right_hand
										&& bugObj != xch->right_hand
										&& xch->right_hand->o.radio.channel
												== tobj->o.radio.channel
										&& xch->right_hand->o.radio.encrypt
												== tobj->o.radio.encrypt) {
									if (get_obj_affect(xch->right_hand,
									MAGIC_SENT))
										heard = true;
									else
										duplicated = true;
							...
							if (xch->left_hand
									&& GET_ITEM_TYPE(xch->left_hand)
											== ITEM_E_RADIO) {
								if (xch->left_hand->o.radio.status != 0
										&& tobj != xch->left_hand
										&& bugObj != xch->left_hand
										&& xch->left_hand->o.radio.channel
												== tobj->o.radio.channel
										&& xch->left_hand->o.radio.encrypt
												== tobj->o.radio.encrypt) {
									if (get_obj_affect(xch->left_hand,
									MAGIC_SENT))
										heard = true;
									else
										duplicated = true;
							...
							if (xch->equip) {
								for (robj = xch->equip; robj;
										robj = robj->next_content) {
									if (robj->o.radio.status != 0
											&& tobj != robj && bugObj != robj &&
					...
					if (rch) {
						if (tobj != get_equip(rch, WEAR_EAR))
							magic_add_obj_affect(tobj, MAGIC_SENT, -1, 0, 0, 0,
									0);
					} else
					...
					j = static_it(tobj, buf4, buf4, false,
							(int) (dist_penalty * 1.75));
					...
					for (tch = room->people; tch; tch = tch->next_in_room) {
						...
						if (duplicated) {
							sprintf(buf6,
									"Echoed by other radios in the room, ");
						} else
							*buf6 = '\0';
						...
						if (!IS_SET(tch->room->room_flags, OOC)
								&& (!bugObj->o.radio.encrypt
										|| (tobj->o.radio.encrypt
												== bugObj->o.radio.encrypt))
								&& (decipher_speaking(tch, ch->speaks,
										ch->skills[ch->speaks]))) {
							sprintf(buf, "%s%s$p%s says in a %s,", buf6,
									statics[static_index], buf5,
									(GET_SEX (ch) == SEX_MALE ?
						...
						} else if (tobj->o.radio.encrypt
								!= bugObj->o.radio.encrypt) {
							sprintf(buf, "%s%s$p%s says something,", buf6,
									statics[static_index], buf5);

							sprintf(buf + strlen(buf),
									" but the words are scrambled and distorted beyond comprehension.");
							deciphered = false;
						} else if (!IS_SET(tch->room->room_flags, OOC)) {
							sprintf(buf, "%s%s$p%s says something in a %s,",
									buf6, statics[static_index], buf5,
									(GET_SEX (ch) == SEX_MALE ?
											"male voice" : "female voice"));

							sprintf(buf + strlen(buf),
									" but you are unable to decipher the words.");
							deciphered = false;
						} else {
							sprintf(buf, "%s%s$p%s says in a %s,", buf6,
									statics[static_index], buf5,
...
void personalize_temote_string(CHAR_DATA* src, CHAR_DATA* tar, char* emote) {
	char desc[MAX_STRING_LENGTH] = { '\0' };
	char output[MAX_STRING_LENGTH] = { '\0' };
	char buf[MAX_STRING_LENGTH] = { '\0' };
	CHAR_DATA *tch = NULL, *target = NULL;
	...
	*output = '\0';
	...
	if (target) {
		...
		if (!are_grouped(target, src) && !GET_TRUST(target)
				&& (!af || !is_at_table(target, af->a.table.obj))) {
...
void personalize_temote(CHAR_DATA* src, char* emote) {
	char desc[MAX_STRING_LENGTH] = { '\0' };
	char copy[MAX_STRING_LENGTH] = { '\0' };
	CHAR_DATA* tch = NULL;
	AFFECTED_TYPE* af = NULL;
	...
	for (tch = src->room->people; tch; tch = tch->next_in_room) {
		...
		af = (get_affect(src, MAGIC_SIT_TABLE));
		if (!are_grouped(tch, src) && !GET_TRUST(tch)
				&& (!af || !is_at_table(tch, af->a.table.obj))) {
			char speech[MAX_STRING_LENGTH] = { '\0' };
			char heard[MAX_STRING_LENGTH] = { '\0' };
			int j = 0;
...
void reply_reset(CHAR_DATA* ch, CHAR_DATA* target, char* buf, int cmd) {
	...
	for (reset = target->mob->resets; reset; reset = reset->next) {
		...
		for (argument = one_argument(keywords, buf2); *buf2; argument =
...
void do_say(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (cmd == 2) {
		...
		if (!(radio = get_obj_in_dark(ch, buf, ch->right_hand)) && !(radio =
				get_obj_in_dark(ch, buf, ch->left_hand)) && !(radio =
				get_equip_arg(ch, WEAR_EAR, buf))
				&& !(radio = get_equip_arg(ch, WEAR_WRIST_L, buf)) && !(radio =
						get_equip_arg(ch, WEAR_WRIST_R, buf)) && !(radio =
		...
		if (radio->o.elecs.status == 0) {
			act("$p is not switched on.", false, ch, radio, 0,
			TO_CHAR | _ACT_FORMAT);
			return;
		}

		if (!IS_SET(radio->o.elecs.elec_bits, ELEC_FEA_RADIO_SET)
				&& radio->o.radio.channel >= 1
				&& radio->o.radio.channel <= 100) {
			act(
					"$p is set to a listen-only channel: it is unable to broadcast over this frequency.",
					false, ch, radio, 0, TO_CHAR | _ACT_FORMAT);
			return;
		}

		if (IS_SET(radio->o.elecs.elec_bits, ELEC_FEA_RADIO_ONEWAY)) {
			act(
					"$p is not capable of making transmissions - only receiving them.",
					false, ch, radio, 0, TO_CHAR | _ACT_FORMAT);
			return;
		}
	...
	for (tch = vnum_to_room(ch->in_room)->people; tch; tch =
			tch->next_in_room) {
		...
		if ((af_table && !is_at_table(tch, af_table->a.table.obj))
			...
			if ((tch->skills[ch->speaks]
					&& decipher_speaking(tch, ch->speaks,
							ch->skills[ch->speaks]) && heard_something)
		...
		if (cmd == 0 || cmd == 1) {
			if (!IS_SET(ch->room->room_flags, OOC)
				...
				sprintf(buf, "$N%s %ss,", buf3,
						utters[cmd]/*, (tch->skills[ch->speaks] || tongues) ? skills[ch->speaks] : "an unknown tongue"*/);
				/*if (tch->skills[ch->speaks] >= 50
				 && ch->skills[ch->speaks] < 50)
				 sprintf (buf + strlen (buf), " %s,",
				 accent_desc (ch, ch->skills[ch->speaks]));
			...
			if (tch == target) {
				if (!IS_SET(ch->room->room_flags, OOC)
						&& decipher_speaking(tch, ch->speaks,
								ch->skills[ch->speaks])) {
					sprintf(buf, "$N%s %ss you,", buf3, utters[cmd]/*, (tch->skills[ch->speaks] || tongues) ?
					...
					sprintf(buf, "$N%s %ss you something,", buf3, utters[cmd]/*, (tch->skills[ch->speaks] || tongues) ?
					 skills[ch->speaks] : "an unknown tongue"*/);
					if (*voice)
						sprintf(buf + strlen(buf), " %s,", voice);
					sprintf(buf + strlen(buf),
				...
				if (!IS_SET(ch->room->room_flags, OOC)
					...
					sprintf(buf, "$N%s %ss %s something", buf3, utters[cmd],
							char_short(target)/*,
			...
			if (tch->room == ch->room) {
				if (!IS_SET(ch->room->room_flags, OOC)
						&& decipher_speaking(tch, ch->speaks,
								ch->skills[ch->speaks])) {
					sprintf(buf, "$N%s %ss in to $p,", buf3, utters[cmd]/*, (tch->skills[ch->speaks] || tongues) ?
					 skills[ch->speaks] : "an unknown tongue"*/);
					deciphered = true;
					/*if (tch->skills[ch->speaks] >= 50
					 && ch->skills[ch->speaks] < 50)
			...
			if (!IS_SET(ch->room->room_flags, OOC)
				...
				sprintf(buf, "$N%s %ss something,", buf3, utters[cmd]/*,
				 (tch->skills[ch->speaks] || tongues) ?
				 skills[ch->speaks] : "an unknown tongue"*/);
				if (*voice)
					sprintf(buf + strlen(buf), " %s,", voice);
		...
		for (tch = next_room->people; tch; tch = tch->next_in_room) {
			if ((waf = get_affect(tch, AFFECT_LISTEN_DIR))) {
				if (rev_dir[dir]
					...
					if (GET_TRUST(
							tch) && !IS_NPC(tch) && GET_FLAG(tch, FLAG_SEE_NAME))
						sprintf(buf3, " (%s)", GET_NAME(ch));
					else
						*buf3 = '\0';
					...
					heard_something = whisper_it(tch, buf2, buf2,
							dist_mod * 15);
					...
					if (cmd == 0 || cmd == 1) {
						if (!IS_SET(ch->room->room_flags, OOC)
								...
								&& decipher_speaking(tch, ch->speaks,
										ch->skills[ch->speaks])) {
							sprintf(buf, "To the %s, you overhear $N%s %s,",
							...
							if (*voice)
								...
								&& !heard_something) {
							sprintf(buf,
									"To the %s, you overhear $N%s %s something",
									direction[rev_dir[waf->a.shadow.edge]],
									buf3,
									utters[cmd]/*,(tch->skills[ch->speaks] || tongues) ? skills[ch->speaks] : "an unknown tongue"*/);
							if (*voice)
						...
						if (!IS_SET(ch->room->room_flags, OOC)
								...
								&& decipher_speaking(tch, ch->speaks,
										ch->skills[ch->speaks])) {
							sprintf(buf, "To the %s, you overhear $N%s %s %s,",
									direction[rev_dir[waf->a.shadow.edge]],
									buf3, utters[cmd], char_short(target)/*,
									 (tch->skills[ch->speaks]
									 || tongues) ? skills[ch->
									 speaks] :
									 "an unknown tongue"*/);
							...
							if (*voice)
								...
								&& !heard_something) {
							sprintf(buf,
									"To the %s, you overhear $N%s %s %s something",
									direction[rev_dir[waf->a.shadow.edge]],
									buf3, utters[cmd], char_short(target)/*,
									 (tch->skills[ch->speaks] || tongues) ?
									 skills[ch->speaks] : "an unknown tongue"*/);
						...
						if (!IS_SET(ch->room->room_flags, OOC)
								...
								&& decipher_speaking(tch, ch->speaks,
										ch->skills[ch->speaks])) {
							sprintf(buf,
									"To the %s, you overhear $N%s %s in to $p,",
									direction[rev_dir[waf->a.shadow.edge]],
									buf3, utters[cmd]/*, (tch->skills[ch->speaks] || tongues) ?
									 skills[ch->speaks] : "an unknown tongue"*/);
							deciphered = true;
							/*if (tch->skills[ch->speaks] >= 50
							 && ch->skills[ch->speaks] < 50)
							...
							if (*voice)
								...
								&& !heard_something) {
							sprintf(buf,
									"To the %s, you overhear $N%s %s something in to $p,",
									direction[rev_dir[waf->a.shadow.edge]],
									buf3, utters[cmd]/*, (tch->skills[ch->speaks] || tongues) ?
									 skills[ch->speaks] : "an unknown tongue"*/);
							if (*voice)
	...
	*buf4 = toupper(*buf4);
	...
	if (cmd == 2) {
		if (*voice) {
			if (!IS_SET(ch->room->room_flags, OOC))
				sprintf(buf, "You %s #5%s#0, %s,\n   \"%s\"\n", utters[cmd],
						char_short(target), //skills[ch->speaks],
						voice, buf4);
			else
				sprintf(buf, "You %s #5%s#0, %s,\n   \"%s\"\n", utters[cmd],
		...
		if (*voice) {
			...
			if (!IS_SET(ch->room->room_flags, OOC))
				...
				sprintf(buf, "You %s in to #2%s#0, %s,\n   \"%s\"\n",
						utters[cmd], obj_short_desc(radio), voice, buf4);
		} else {
			send_monitor_radio(ch, buf4, radio->o.radio.channel); // Send this to listening staff - Shade
			if (!IS_SET(ch->room->room_flags, OOC))
				sprintf(buf, "You %s in to #2%s#0,\n   \"%s\"\n", utters[cmd],
						obj_short_desc(radio), // skills[ch->speaks],
						buf4);
			else
				sprintf(buf, "You %s in to #2%s#0,\n   \"%s\"\n", utters[cmd],
			...
			for (tobj = object_list; tobj; tobj = tobj->next) {
				...
				if (!heard && !duplicated) {
					for (xch = room->people; xch; xch = xch->next_in_room) {
						...
						if (xch->left_hand
								&& GET_ITEM_TYPE(xch->left_hand) == ITEM_E_RADIO) {
							if (xch->left_hand->o.radio.status != 0
									&& tobj != xch->left_hand
									&& bugObj != xch->left_hand
									&& xch->left_hand->o.radio.channel
				...
				j = static_it(tobj, buf4, buf4, radio_station,
						(int) (dist_penalty * 1.75));
				...
				for (tch = room->people; tch; tch = tch->next_in_room) {
					...
					if (!IS_SET(tch->room->room_flags, OOC)
							...
							&& (decipher_speaking(tch, ch->speaks,
									ch->skills[ch->speaks]))) {
						if (*voice)
							sprintf(buf, "%s%s$p%s %ss, %s,", buf6,
									statics[static_index], buf5, utters[cmd],
									voice);
						else
							sprintf(buf, "%s%s$p%s %ss in a %s,", buf6,
									statics[static_index], buf5, utters[cmd],
									(GET_SEX (ch) == SEX_MALE ?
											"male voice" : "female voice"));
						deciphered = true;
						...
						if (*voice)
							sprintf(buf, "%s%s$p%s %ss something %s,", buf6,
									statics[static_index], buf5, utters[cmd],
									voice);
						else
							sprintf(buf, "%s%s$p%s %ss something in a %s,",
									buf6, statics[static_index], buf5,
									utters[cmd],
									(GET_SEX (ch) == SEX_MALE ?
											"male voice" : "female voice"));
	...
	if (cmd == 6) {
		...
		for (tobj = object_list; tobj; tobj = tobj->next) {
			...
			if (!heard && !duplicated) {
				for (xch = room->people; xch; xch = xch->next_in_room) {
					if (xch->right_hand
							&& GET_ITEM_TYPE(xch->right_hand) == ITEM_E_RADIO) {
						if (xch->right_hand->o.radio.status != 0
								&& tobj != xch->right_hand
								&& radio != xch->right_hand
								&& xch->right_hand->o.radio.channel
										== tobj->o.radio.channel
								&& xch->right_hand->o.radio.encrypt
										== tobj->o.radio.encrypt) {
			...
			j = static_it(tobj, buf4, bufScrambled, radio_station,
					dist_penalty);
			...
			for (tch = room->people; tch; tch = tch->next_in_room) {
				...
				if (!IS_SET(tch->room->room_flags, OOC)
					...
					if (*voice)
						...
						sprintf(buf, "%s%s$p%s %ss something in a %s,", buf6,
								statics[static_index], buf5, utters[cmd],
								(GET_SEX (ch) == SEX_MALE ?
										"male voice" : "female voice"));
	...
	if (ch->in_room == StageRoom) {
		for (index = 0; index < 4; index = index + 1) {
			for (tch = vnum_to_room(AudienceRoom[index])->people; tch;
				...
				if (cmd == 0 || cmd == 1 || cmd == 2 || cmd == 3) {
					if (!IS_SET(ch->room->room_flags, OOC)
							&& decipher_speaking(tch, ch->speaks,
									ch->skills[ch->speaks])) {
						sprintf(buf, "On the stage, $N%s %ss,", buf3,
								utters[cmd]);
						...
						sprintf(buf, "On the stage, $N%s %ss something,", buf3,
								utters[cmd]);
...
void do_ichat(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (!(*argument)) {
		...
		if (!ch_wiznet_set) {
			send_to_char("You are not currently tuned into the wiznet. "
		...
		if (IS_NPC (ch) && ch->descr()->original) {
			sprintf(buf1, "#1[Wiznet: %s (%s)]#0 %s\n",
					GET_NAME(ch->descr()->original), GET_NAME(ch),
					CAP(argument));
		} else {
				...
				*s_buf = '\0';
				...
				if (IS_SET(i->character->act, PLR_QUIET)) {
					...
					if (!IS_MORTAL(i->character)) {
						CHAR_DATA* tch =
								(i->original) ? (i->original) : (i->character);
						sprintf(s_buf,
...
void do_helpline(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (!(*argument)) {
		send_to_char(
				"The following Accounts are currently connected to the Helpline:\n\n",
				ch);
		...
		for (i = descriptor_list; i; i = i->next)
			if (i->character && GET_FLAG(i->character, FLAG_NEWBNET)) {
				if (!i->connected) {
					*s_buf = '\0';
					if (!IS_SET(i->character->act,
							PLR_QUIET) && GET_FLAG (i->character, FLAG_NEWBNET)) {
						send_to_char(p, i->character);
					} else {
						if (IS_SET(i->character->act, PLR_QUIET))
...
void do_whisper(CHAR_DATA* ch, char* argument, int cmd) {
	...
	*argument = toupper(*argument);
	...
	if (!tongues
			&& !real_skill(ch,
					ch->speaks) && !IS_SET (ch->room->room_flags, OOC)) {
	...
	*buf3 = toupper(*buf3);
	...
	for (tch = vnum_to_room(ch->in_room)->people; tch; tch =
		...
		if (!heard_something) {
			sprintf(buf,
					"$n whispers something to $3, %s but you can't quite make out the words.",
					buf5);
		...
		if (tch == vict) {
			if (!IS_SET(ch->room->room_flags, OOC)
				...
				sprintf(buf,
						...
						/*(tch->skills[ch->speaks]
						 || tongues) ? skills[ch->
						 speaks] : "an unknown tongue",*/buf5, HSHR(ch));
				act(buf, false, tch, (OBJ_DATA *) ch, 0,
				TO_CHAR | _ACT_FORMAT);
			} else {
				sprintf(buf, "$3 whispers to you, %s \n   \"%s\"", buf5, buf4);
...
void do_petition(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (ch->descr()->acct
		...
		act("Your ability to petition has been revoked by an admin.", false, ch,
				0, 0, TO_CHAR);
		return;
	}
...
void do_shout(CHAR_DATA* ch, char* argument, int cmd) {
	...
	for (tch = vnum_to_room(ch->in_room)->people; tch; tch =
		...
		if (!decipher_speaking(tch, ch->speaks,
				ch->skills[ch->speaks]) && !IS_SET (ch->room->room_flags, OOC)) {
			sprintf(buf + strlen(buf), " something that you fail to decipher.");
			act(buf, false, tch, 0, ch, TO_CHAR | _ACT_FORMAT);
		} else if (tch->descr()) {
			act(buf, false, tch, 0, ch, TO_CHAR | _ACT_FORMAT);
				...
				if (lookup_race_int(ch->race, RACE_NOMAD)) {
					if (!IS_SET(ch->room->room_flags, OOC)) {
						if (!tch->skills[ch->speaks])
							...
							sprintf(buf,
									"You hear a bestial voice shout from %s",
									rev_d[door]);
					} else
						sprintf(buf, "You hear a bestial voice shout from %s",
								rev_d[door]);
					...
					if (!IS_SET(ch->room->room_flags, OOC)) {
						if (!tch->skills[ch->speaks])
							...
							sprintf(buf,
									"You hear a metallic voice shout from %s",
									rev_d[door]);
					} else
						sprintf(buf, "You hear a metallic voice shout from %s",
								rev_d[door]);
					...
					if (!IS_SET(ch->room->room_flags, OOC)) {
						if (!tch->skills[ch->speaks])
							...
							sprintf(buf,
									"You hear a female voice shout from %s",
									rev_d[door]);
					} else
			...
			for (tch = room->people; tch; tch = tch->next_in_room) {
				...
				if (lookup_race_int(ch->race, RACE_NOMAD)) {
					...
					if (!IS_SET(ch->room->room_flags, OOC)) {
						...
						sprintf(buf,
								"You hear a voice shout from the outside,");
				}
				if (tch->skills[ch->speaks] >= 50
						&& ch->skills[ch->speaks] < 50)
					sprintf(buf + strlen(buf), " %s,",
			...
			for (tch = vnum_to_room(AudienceRoom[index])->people; tch;
				...
				if (cmd == 0 || cmd == 1 || cmd == 2 || cmd == 3) {
					if (!IS_SET(ch->room->room_flags, OOC)
							&& decipher_speaking(tch, ch->speaks,
									ch->skills[ch->speaks])) {
						sprintf(buf, "On the stage, $N%s shouts,", buf3);
						...
						sprintf(buf, "On the stage, $N%s shouts something,",
								buf3);
...
int add_to_list(ROOM_DATA** list, ROOM_DATA* room, int* elements) {
	int i = 0;
...
void get_room_list(int radius, ROOM_DATA* room, ROOM_DATA** rooms, int dists[],
		int* num_rooms) {
	int room_set_top = 0;
	int room_set_bot = 0;
	int dir = 0;
		...
		while (room_set_top <= room_set_bot) {
			for (dir = 0; dir <= 5; dir++)
				if (rooms[room_set_top]->dir_option[dir])
					add_to_list(rooms,
							vnum_to_room(
									rooms[room_set_top]->dir_option[dir]->to_room),
							num_rooms);
...
void wolf_howl(CHAR_DATA* ch) {
	...
	for (i = 1; i < num_rooms; i++) {
		if (rooms[i]->people) {
			...
			sprintf(buf, "There is a loud, mournful howl coming from the %s.\n",
					dirs[dir]);
		...
		for (tch = rooms[i]->people; tch; tch = tch->next_in_room) {
			...
			act("$n stands upright suddenly, cocking $s head.", true, tch, 0, 0,
			TO_ROOM);
...
void spider_screech(CHAR_DATA* ch) {
	int num_rooms = 0;
	int dists[220];
	int i = 0;
	int dir = 0;
	...
	for (i = 1; i < num_rooms; i++) {
		...
		for (tch = rooms[i]->people; tch; tch = tch->next_in_room) {
			...
			act("$n turns suddenly, cocking $s head.", true, tch, 0, 0,
			TO_ROOM);
...
void warg_howl(CHAR_DATA* ch) {
	...
	for (i = 1; i < num_rooms; i++) {
		if (rooms[i]->people) {
			...
			sprintf(buf, "There is a blood-curdling howl coming from the %s.\n",
					dirs[dir]);
		...
		for (tch = rooms[i]->people; tch; tch = tch->next_in_room) {
			...
			act("$n stands upright suddenly, and bares $s teeth.", true, tch, 0,
					0, TO_ROOM);
...
void insect_smell(CHAR_DATA* ch) {
	int num_rooms = 0;
	int dists[220];
	int i = 0;
	int dir = 0;
...
bool evaluate_emote_string(CHAR_DATA* ch, std::string* first_person,
		std::string third_person, std::string argument) {
	OBJ_DATA* object;
	CHAR_DATA* tch = NULL;
	int i = 1;
	...
	if (argument[0] == '(') {
		for (tch = ch->room->people; tch; tch = tch->next_in_room) {
			...
			while (argument[i] != ')') {
				if (argument[i] == '\0') {
					send_to_char(
							"Incorrect usage of emote string - please see HELP EMOTE\n",
							ch);
					return false;
				}
				...
				if (argument[i] == '*') {
					...
					if (!(object = get_obj_in_list_vis(ch, key_string.c_str(),
							ch->right_hand))
							&& !(object = get_obj_in_list_vis(ch,
									key_string.c_str(), ch->left_hand))
							&& !(object = get_obj_in_list_vis(ch,
									key_string.c_str(), ch->room->contents))
							&& !(object = get_obj_in_list_vis(ch,
									key_string.c_str(), ch->equip))) {
						error_string =
								"You cannot find an object with the keyword [#2";
				...
				if (argument[i] == '~') {
					...
					if (!get_char_room_vis(ch, key_string.c_str())) {
						error_string =
								"You cannot find a person with the keyword [#5";
						error_string.append(key_string);
						error_string.append("#0]\n");
						send_to_char(error_string.c_str(), ch);
						return false;
		...
		const_to_non_const_cstr(first_person->append(".").c_str(),
				non_const_first);
		char non_const_third[MAX_STRING_LENGTH] = "";
		const_to_non_const_cstr(third_person.append(".").c_str(),
				non_const_third);
...
void do_plan(CHAR_DATA* ch, char* argument, int cmd) {
	...
	if (argument && *argument) {
		...
		// change the short-term plan
			...
			// (re)set the short-term plan
				...
				// bad plan message size
					...
					send_to_char(
							"Your short-term plan must be less than eighty characters in length.\nTo clear your plan, type #6plan short clear#0.\n",
							ch);
				}
			}
		}
		...
		// change the long-term plan
			...
			// (re)set the long-term plan
				...
				// bad message size
					...
					send_to_char(
							"Your long-term plan must be at most three lines.\nTo clear your plan, type #6plan long clear#0.\n",
							ch);
				}
			}
		} else {
		...
		if ((ch->plan && !ch->plan->empty())
				|| (ch->goal && !ch->goal->empty())) {
