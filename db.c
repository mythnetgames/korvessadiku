...
int can_use_ritual_magic(struct char_data *ch) {
	/* Only staff/GM can use ritual magic */
	return (IS_STAFF(ch) || IS_GM(ch));
}

int can_use_high_tier_power(struct char_data *ch) {
	/* Only allowed via story event, not by default */
	return 0;
}

void attempt_magic(struct char_data *ch) {
	if (!can_use_magic(ch)) {
		printf("\033[38;5;196mMagic is not available to players in Korvessa.\033[0m\n");
		return;
	}
	/* ...existing magic logic if allowed... */
}

void attempt_ritual_magic(struct char_data *ch) {
	if (!can_use_ritual_magic(ch)) {
		printf("\033[38;5;208mRitual magic is only available to staff or via quests.\033[0m\n");
		return;
	}
	/* ...existing ritual magic logic if allowed... */
}

void attempt_high_tier_power(struct char_data *ch) {
	if (!can_use_high_tier_power(ch)) {
		printf("\033[38;5;201mHigh-tier powers are only available through story events.\033[0m\n");
		return;
	}
	/* ...existing high-tier power logic if allowed... */
}
...
void korvessa_character_creation(struct char_data *ch) {
	char input[128];
	int stat[6] = {9,9,9,9,9,9};
	int domain_caps[8] = {40,40,40,40,40,40,40,40};
	int standing[8] = {0,0,0,0,0,0,0,0};
	...
	const char *HEADER = "\033[38;5;33m";      /* Blue */
	const char *STEP = "\033[38;5;39m";        /* Cyan */
	const char *PROMPT = "\033[38;5;70m";      /* Green */
	const char *CHOICE = "\033[38;5;208m";     /* Orange */
	const char *INFO = "\033[38;5;141m";       /* Violet */
	const char *DIVIDER = "\033[38;5;244m";    /* Grey */
	const char *FACT = "\033[38;5;226m";       /* Yellow */
	const char *COMPLETE = "\033[38;5;82m";    /* Bright green */
	...
	printf("%sChoose your race:%s ", PROMPT, RESET);
	fgets(input, sizeof(input), stdin);
	set_race(ch, input);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	/* Personality selection */
	printf("%s%s[Step 2] Personality Archetype%s\n", BOLD, STEP, RESET);
	printf("%sChoose your personality:%s\n", PROMPT, RESET);
	printf("%sStalwart, Sharp-Eyed, Artificer, Silver-Tongued, Hidden, Devoted, Insightful, Freehands%s\n", CHOICE, RESET);
	printf("%sYour choice:%s ", PROMPT, RESET);
	fgets(input, sizeof(input), stdin);
	set_personality(ch, input);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	/* Stat assignment */
	printf("%s%s[Step 3] Stat Assignment%s\n", BOLD, STEP, RESET);
	printf("%sAssign your stats (STR DEX CON INT WIS CHA), space-separated:%s ", PROMPT, RESET);
	fgets(input, sizeof(input), stdin);
	sscanf(input, "%d %d %d %d %d %d", &stat[0], &stat[1], &stat[2], &stat[3], &stat[4], &stat[5]);
	assign_stats(ch, stat[0], stat[1], stat[2], stat[3], stat[4], stat[5]);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	/* Skill group buy */
	printf("%s%s[Step 4] Skill Domains%s\n", BOLD, STEP, RESET);
	printf("%sChoose up to 3 skill domains to start:%s\n", PROMPT, RESET);
	printf("%s0-Combat, 1-Stealth, 2-Social, 3-Crafting, 4-Survival, 5-Lore, 6-Medical, 7-Other%s\n", CHOICE, RESET);
	printf("%sYour choices (space-separated):%s ", PROMPT, RESET);
	fgets(input, sizeof(input), stdin);
	int d1, d2, d3;
	sscanf(input, "%d %d %d", &d1, &d2, &d3);
	buy_skill_group(ch, d1, 40); buy_skill_group(ch, d2, 40); buy_skill_group(ch, d3, 40);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	/* Standing initialization */
	printf("%s%s[Step 5] Faction Standing%s\n", BOLD, STEP, RESET);
	printf("%sSet your initial standing for 8 factions (space-separated, -2000 to +2000):%s\n", PROMPT, RESET);
	printf("%sCivic_Order, Laborers, Merchants, Nobility, Underbelly, Watcher_Cult, Scholars, Feyliks_Freefolk%s\n", CHOICE, RESET);
	printf("%sYour values:%s ", PROMPT, RESET);
	fgets(input, sizeof(input), stdin);
	sscanf(input, "%d %d %d %d %d %d %d %d", &standing[0], &standing[1], &standing[2], &standing[3], &standing[4], &standing[5], &standing[6], &standing[7]);
	for (int i = 0; i < 8; i++) set_standing(ch, i, standing[i]);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	/* Background entry */
	printf("%s%s[Step 6] Character Background%s\n", BOLD, STEP, RESET);
	printf("%sDescribe your character's background:%s ", PROMPT, RESET);
	fgets(input, sizeof(input), stdin);
	set_background(ch, input);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	/* Public knowledge entry */
	char facts[10][128];
	int fact_count = 0;
	printf("%s%s[Step 7] Public Knowledge%s\n", BOLD, STEP, RESET);
	printf("%sEnter up to 10 public knowledge facts. Type 'done' to finish.%s\n", PROMPT, RESET);
	while (fact_count < 10) {
		printf("%sFact %d:%s ", FACT, fact_count + 1, RESET);
		fgets(facts[fact_count], sizeof(facts[fact_count]), stdin);
		if (strncmp(facts[fact_count], "done", 4) == 0) break;
		fact_count++;
	}
	char all_facts[1024] = "";
	for (int i = 0; i < fact_count; i++) {
		strncat(all_facts, facts[i], sizeof(all_facts) - strlen(all_facts) - 2);
		if (i < fact_count - 1) strncat(all_facts, "; ", sizeof(all_facts) - strlen(all_facts) - 2);
	}
	set_public_knowledge(ch, all_facts);
	printf("%s--------------------------------%s\n\n", DIVIDER, RESET);

	printf("%s%sCharacter creation complete!%s\n", BOLD, COMPLETE, RESET);
}
/* Korvessa Healing & Injury System */
#define KORVESSA_INJURY_MINOR 0
#define KORVESSA_INJURY_MODERATE 1
#define KORVESSA_INJURY_MAJOR 2
#define KORVESSA_INJURY_CRITICAL 3

void apply_bandage(struct char_data *ch) {
	if (ch->injury_tier == KORVESSA_INJURY_MINOR) {
		ch->injury_tier = -1; /* Healed */
	}
}

void apply_first_aid(struct char_data *ch) {
	if (ch->injury_tier == KORVESSA_INJURY_MODERATE) {
		ch->injury_tier = KORVESSA_INJURY_MINOR;
	}
}

void apply_chirurgy(struct char_data *ch) {
	if (ch->injury_tier == KORVESSA_INJURY_MAJOR) {
		ch->injury_tier = KORVESSA_INJURY_MODERATE;
	}
}

void apply_herbalism(struct char_data *ch) {
	if (ch->injury_tier >= KORVESSA_INJURY_MINOR && ch->injury_tier <= KORVESSA_INJURY_MAJOR) {
		ch->stamina += 10; /* Small regen or pain relief */
	}
}

void apply_rest(struct char_data *ch) {
	if (ch->injury_tier == -1) {
		ch->points.hit = ch->points.max_hit;
		ch->stamina = 100;
	}
}
/* Korvessa Skill System Logic */
#define KORVESSA_SKILL_START_CAP 40
#define KORVESSA_SKILL_SELF_TRAIN_CAP 80
#define KORVESSA_SKILL_TEACHER_TRAIN_CAP 90
#define KORVESSA_SKILL_GM_CAP 95

int can_train_skill(struct char_data *ch, int skill_num) {
	struct char_skill_data *sk = &ch->skills[skill_num];
	return sk->learned < sk->proficiency_cap;
}

int train_skill(struct char_data *ch, int skill_num, int xp_cost) {
	struct char_skill_data *sk = &ch->skills[skill_num];
	if (!can_train_skill(ch, skill_num)) return 0;
	if (spend_xp(ch, xp_cost) < xp_cost) return 0;
	int rate = sk->learning_rate > 0 ? sk->learning_rate : 1;
	int gain = rate;
	if (sk->learned + gain > sk->proficiency_cap) gain = sk->proficiency_cap - sk->learned;
	sk->learned += gain;
	return gain;
}

void set_skill_domain(struct char_data *ch, int skill_num, byte domain) {
	ch->skills[skill_num].domain = domain;
}

void set_skill_cap(struct char_data *ch, int skill_num, byte cap) {
	ch->skills[skill_num].proficiency_cap = cap;
}

void set_skill_learning_rate(struct char_data *ch, int skill_num, byte rate) {
	ch->skills[skill_num].learning_rate = rate;
}

void restrict_skill(struct char_data *ch, int skill_num, bool restricted) {
	ch->skills[skill_num].restricted = restricted;
}
/* Korvessa XP Economy */
#define KORVESSA_DAILY_XP_CAP 200

void reset_daily_xp(struct char_data *ch) {
	ch->daily_xp_gained = 0;
	ch->last_xp_reset = time(0);
}

int gain_xp(struct char_data *ch, int amount) {
	time_t now = time(0);
	struct tm *now_tm = localtime(&now);
	struct tm *last_tm = localtime(&ch->last_xp_reset);
...
void store_to_char(struct char_file_u *st, struct char_data *ch)
{
	...
	ch->player.height = st->height;
	 ...
	 /* Korvessa additions */
	 strncpy(ch->personality, st->personality, sizeof(ch->personality));
	 for (i = 0; i < 8; i++) ch->standing[i] = st->standing[i];
	 strncpy(ch->public_knowledge, st->public_knowledge, sizeof(ch->public_knowledge));
	 ch->stamina = st->stamina;
	 ch->injury_tier = st->injury_tier;
	 for (i = 0; i < 8; i++) ch->skill_domain_caps[i] = st->skill_domain_caps[i];
...
void char_to_store(struct char_data *ch, struct char_file_u *st)
{
	...
	st->level    = GET_LEVEL(ch);
	   ...
	   /* Korvessa additions */
	   strncpy(st->personality, ch->personality, sizeof(st->personality));
	   for (i = 0; i < 8; i++) st->standing[i] = ch->standing[i];
	   strncpy(st->public_knowledge, ch->public_knowledge, sizeof(st->public_knowledge));
	   st->stamina = ch->stamina;
	   st->injury_tier = ch->injury_tier;
	   for (i = 0; i < 8; i++) st->skill_domain_caps[i] = ch->skill_domain_caps[i];
...
void set_personality(struct char_data *ch, const char *personality) {
	strncpy(ch->personality, personality, sizeof(ch->personality));
}
...
void buy_skill_group(struct char_data *ch, int domain, int cap) {
	ch->skill_domain_caps[domain] = cap;
}

void set_standing(struct char_data *ch, int faction, int value) {
	ch->standing[faction] = value;
}

void set_background(struct char_data *ch, const char *background) {
	strncpy(ch->player.long_descr, background, 80);
}
...
void init_char(struct char_data *ch)
{
	...
	for (i = 0; i < 8; i++) ch->standing[i] = 0; /* Neutral standing */
	strncpy(ch->public_knowledge, "", sizeof(ch->public_knowledge));
	ch->stamina = 100;
	ch->injury_tier = 0;
	for (i = 0; i < 8; i++) ch->skill_domain_caps[i] = 40; /* Starting cap */
